...
Outputs (results/p4a/): p4a_replay_{split}_B0XX.csv (200 rows) + contextual_bandit.csv + PROVENANCE_p4a.txt.
Run:  /path/to/env/python baselines/contextual_bandit/evaluate.py
"""
import hashlib
import json
import os
...
def load_bandits():
    if not os.path.exists(P4A_MANIFEST):
        raise SystemExit('P4-A eval FUSE: P4A_MANIFEST.json absent -- run train.py first.')

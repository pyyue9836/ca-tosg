...
# Per-frame F1 (all boxes, score=ones -- matches canonical_f1.py) + global-sort AP.
"""
Modes:
  --mode score : for every gs_rerun/jscc_v3/jscc_<ch>_<split>_snr<NN>.npz, per-frame F1 + global-sort
                 ...
                 results/baselines/importance_map_jscc/jscc_ap_f1.csv and per-frame F1 npz jscc_perframe_f1_<ch>_<split>.npz.
  --mode probe : interpolation-validity check. Reads the probe npz at SNR 8/10/12 (validate), computes
                 ...
                 real F1_10. One row -> results/baselines/importance_map_jscc/interp_probe_mae.csv.
"""
...
JSCC_DIR = os.path.join(GS, 'jscc_v3'); OUT = os.path.join(P1, 'results/baselines/importance_map_jscc')
...
def f1_vs(boxes, canon, iou=0.5):
    """All predicted boxes (score=ones), IoU 0.5 -- identical recipe to canonical_f1.py."""
...
def main():
    ...
    if o.mode == 'probe':
        ...
        print('\n[PROBE]', row); print('wrote results/baselines/importance_map_jscc/interp_probe_mae.csv')
        return
    ...
    print('wrote results/baselines/importance_map_jscc/jscc_ap_f1.csv')

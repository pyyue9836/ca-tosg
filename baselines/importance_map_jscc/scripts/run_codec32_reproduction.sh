...
$RUN baselines/importance_map_jscc/run_jscc_eval.py --config $RCFG --ckpt $RAY_S2 \
  --channel rayleigh --snrs 0,5,10,15,20 --max_samples 1000 --num_workers 4 --tag rayleigh_jscc || true
$RUN baselines/importance_map_jscc/run_jscc_eval.py --config $OCFG --ckpt $OFDM_S2 \
...
for c in awgn rayleigh ofdm; do
  $RUN projects/ca_tosg/evaluation/figures/plot_paper_figures.py --channel $c \

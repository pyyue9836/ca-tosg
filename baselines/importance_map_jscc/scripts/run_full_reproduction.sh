...
# ---- AWGN JSCC sweep ----
$RUN baselines/importance_map_jscc/run_jscc_eval.py --config $ACFG --ckpt $AWGN_S2 \
...
train_and_sweep () {
  ...
  echo "=== [$CH] stage1 ==="
  $RUN baselines/importance_map_jscc/stage1_pretrain_jscc_reconstruction_sttopk.py \
    --yaml_path $CFG --warm_ckpt $W2C/net_epoch50.pth \
    --out_dir $S1DIR --max_steps 1200
  echo "=== [$CH] stage2 ==="
  $RUN baselines/importance_map_jscc/stage2_whole_network_map_jscc.py \
    --yaml_path $CFG --stage1_ckpt $S1 --out_dir $S2DIR \
    --max_steps 3000 --lambda_rec 0.5
  echo "=== [$CH] sweep ==="
  $RUN baselines/importance_map_jscc/run_jscc_eval.py --config $CFG --ckpt $S2 \
...
$RUN baselines/importance_map_jscc/run_separate_coding_sweep.py --w2c_dir $W2C --qam 16 \
  --snrs 0,4,6,8,10,12,16 --max_samples 1000 --num_workers 4 --tag ldpc16qam || true
$RUN baselines/importance_map_jscc/run_separate_coding_sweep.py --w2c_dir $W2C --qam 256 \
  --snrs 0,12,16,18,20,22,26 --max_samples 1000 --num_workers 4 --tag ldpc256qam || true
$RUN baselines/importance_map_jscc/run_separate_coding_sweep.py --w2c_dir $W2C --qam none \
...
for CH in awgn rayleigh ofdm; do
  $RUN projects/ca_tosg/evaluation/figures/plot_paper_figures.py --channel $CH \

...
def load_manifest(arm):
    path = T.MANIFEST_DGATE if arm == 'dgate' else T.MANIFEST
    want = f'catosg-r21a{"2" if arm == "dgate" else ""}-manifest/1'
    if not os.path.exists(path):
        raise SystemExit(f'R21-A eval FUSE: {os.path.basename(path)} absent -- run --train first.')
    man = json.load(open(path))
    if man.get('schema') != want:
...
def main(arm=None):
    arm = T.parse_arm() if arm is None else arm
    pre = 'r21a2' if arm == 'dgate' else 'r21a'
    out_summ = OUT_SUMM.replace('two_gate.csv', 'two_gate_dgate.csv') if arm == 'dgate' else OUT_SUMM
    out_act = (OUT_ACT.replace('two_gate_actions.csv', 'two_gate_dgate_actions.csv')
               if arm == 'dgate' else OUT_ACT)
    out_prov = PROV.replace('_r21a.txt', '_r21a2.txt') if arm == 'dgate' else PROV
    os.makedirs(OUT_RUNS, exist_ok=True)
    man = load_manifest(arm)
    ...
    for split in D.SPLITS:
        ...
        for tag in tags:
            ...
            tD = -np.inf if bd.get('tau_D') is None else float(bd['tau_D'])
            hot = (r_2d >= tF) if arm != 'dgate' else ((r_2d >= tF) & (d[None, :] >= tD))
            tg_idx = np.where(d[None, :] <= tE, 0, np.where(hot, 2, 1))              # (R,n)
            ...
            pd.DataFrame(dict(realisation=np.arange(D.N_REPLAY), F1_2G=F['TG'], F1_RF=F['RF'],
                ...
                os.path.join(OUT_RUNS, f'{pre}_replay_{split}_B{int(round(bmax*100)):03d}.csv'), index=False)

            row = dict(split=split, budget=bmax, feasible=True, cue=bd['cue'], sign=bd['sign'],
                       tau_E=bd['tau_E'], tau_D=bd.get('tau_D'), tau_F=bd['tau_F'],
    ...
    with open(out_prov, 'w') as f:
        f.write(f'CA-TOSG {man["protocol"]} -- {man["arm"]}\n'
                f'{man["n_free_scalars"]} free scalars. DESCRIPTIVE + CI ONLY; not deployed; no '
                'decision in either direction.\n' + '=' * 88 + '\n')
        f.write(f'manifest: schema {man["schema"]}; all four '
        ...
        for tag in tags:
            ...
            if bd.get('feasible'):
                f.write(f'  B={tag}: candidate {bd["candidate_index"]} {bd["cue"]}/{bd["sign"]:+d}, '
                        f'tau_E={bd["tau_E"]} (q={bd["tau_E_quantile"]}), tau_D={bd.get("tau_D")}, '
                        f'tau_F={bd["tau_F"]}, '

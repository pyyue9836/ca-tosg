...
**Branch** `p1-phy-rebuild` · **written at** `400bfb6` · **recovery tag** `pre-p0-corrigendum` (the pre-correction state; nothing before it is quotable).
...
The contract is `docs/unified_branch_protocol_v2.md`, hashed section by section into
`results/manifests/V2_PROTOCOL_MANIFEST.json`.

**No mainline GPU runs while any protocol section reads NOT LOCKED** — sections 11 (the supervisor's
12-item regeneration list) and 12 (P1-1…P1-8, P2-1…P2-6) are waiting on text that is not in this
repository and must be pasted in.

Everything below this banner describes the **v1** state. It is accurate for v1 and is not the
current work.
...
| compile (`python tests/test_compile.py`) | main **16 pages**, supplementary 9; 0 errors, 0 undefined refs, 1 overfull (17.6pt) |
...
| ledger (`docs/claims.md`) | **105 claims, 105 filled, 0 unbound** |
...
1. `docs/experiment_protocol.md` — the **live protocol** since the R67 (d) split: §1–§10, the
   pre-registered revision block, Appendices A–F. Everything in it is normative or machine-parsed;
   the blocks code reads are tabulated at the end of the file.
2. `docs/history/protocol_changelog.md` — the dated revision batches **R17 → R67**, split out of the
   file above and carrying a NOT-QUOTABLE banner: every entry is as-of its own day. Read backwards
   from the end for how the current state was arrived at, never for what the current state *is*.
   `tests/test_protocol_reconciliation.py` reads **both** files — a verdict recorded in either still
   blocks a contradicting sentence.
3. `docs/p0_corrigendum.md` — what the corrigendum changed, old vs new, per arm.
4. `docs/assumptions_ledger.md` — every input artefact: physical semantics vs accounting convention.
5. `docs/canonical_quantities.md` — derived quantities and how each is re-derived at gate time.
6. `docs/reproducibility.md` — the six-step chain from raw OPV2V to the delivered PDF.
...
1. **Page count.** Main is 16 pages; the target was 14 (R47 added required content, 15 -> 16). Prose compression, a citation audit
...
2. **Where2comm budget-matched rerun: plan v2 written, awaiting cost approval.**
   `archive/docs-history/where2comm_rerun_plan_v2.md` (R50) supersedes the archived R21-B plan and is current with
   the single-collaborator convention, the frozen scoring chain and the four payload conventions.
   Nothing has been run. **Josh picks a tier: conservative ≈ 15 GPU-h, typical ≈ 22 (recommended),
   worst case ≈ 40.** Note the deliberate conflict recorded in the plan: a budget-matched arm is a
   baseline, so the R45-4 TERMINOLOGY row and the `where2comm-baseline` reconciliation pair must be
   amended in the same commit that lands the first number, or the gates will block it. Nothing in
   the paper depends on the outcome.
...
- **Repository layout**: BEVFormer-style, per `archive/docs-history/RESTRUCTURE_PLAN.md` (the supervisor's text,
  verbatim). Per-file map: `archive/docs-history/RESTRUCTURE_MAP.csv`. `paper1/` no longer exists.
...
| `archive/docs-history/RESTRUCTURE_MAP.csv` | old path → new path for every file, plus 887 literal rewrite points |

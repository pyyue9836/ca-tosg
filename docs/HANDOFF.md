...
**Branch** `p1-phy-rebuild` · **written at** `0bdc5f4` · **recovery tag** `pre-p0-corrigendum` (the pre-correction state; nothing before it is quotable).
...
* **protocol reconcile** (R45-6) — the paper against the *record*, not the data: a claim the
  ...
  replacement must. Four pairs registered (payload anchor, Where2comm-as-baseline, C256 dominance,
  the 100 ms latency budget).

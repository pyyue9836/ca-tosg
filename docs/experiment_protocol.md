...
| **Verification** | `python tools/verify_results.py` — **33 checks, all passing**. Compile: main 16 pages, supplementary 12. |
...
## The change-log lives in `docs/history/protocol_changelog.md`

Split out in R67 (d). This file is the **live protocol**: §1–§10, the pre-registered protocol-revision
block, and Appendices A–F. Everything above is normative or machine-parsed and is read by code:

| block | read by |
|---|---|
| ```` ```json CATOSG-CANDIDATES ```` (§6) | `projects/ca_tosg/models/selector.py`, `tests/test_data_leakage.py` (md5-pinned), `projects/ca_tosg/utils/configs.py` |
| ```` ```json CATOSG-P4A ```` (Change-log P4-A) | `baselines/contextual_bandit/train.py` |
| ```` ```json CATOSG-FEATURE-ABLATION ```` (Appendix E) | `projects/ca_tosg/evaluation/feature_ablation.py` |
| §3 Channel grid, §4 Action set | `projects/ca_tosg/utils/configs.py` (which generates `projects/ca_tosg/configs/`), `tests/test_p3_snr_support.py` |
| Appendix B prediction table | `projects/ca_tosg/utils/configs.py` |

**That is the split rule, and it is not stylistic:** a block that code parses stays here; a dated
batch narrative goes to history. Moving one of the blocks above into the history file breaks a
parser or a manifest hash.

The **dated revision batches R17 → R67** are in `docs/history/protocol_changelog.md`, which carries a
NOT-QUOTABLE banner. Both files together are "the record":
`tests/test_protocol_reconciliation.py` reads **both**, so a verdict recorded in either one still
blocks a contradicting sentence in `main.tex` or `supplementary.tex`.

...
| 6 | verify and compile | `python tools/verify_results.py`, `python tests/test_compile.py` | 35 gate results, `main.pdf` (16 pp.), `supplementary.pdf` (12 pp.), `docs/compile_report.md` |
...
* **Tier 1 --- committed-content consistency**: `python tools/verify_results.py --content-only` runs **19 of the 35** gates (`paragraph insertion`, `claims vs main.tex`, `P3 SNR support`, `intra-repo imports`, `action-set wording`, `generators --check`, `protocol reconcile`, `transport products`, `canonical quantities`, `assumptions ledger`, `p6 numbers vs CSV`, `retired-write sweep`, `sealed held-out`, `cue field whitelist`, `provisional exclusion`, `conclusion fidelity`, `paper numbers macros`, `fingerprint coverage`, `stale-fingerprint exit`); the other **16 are skipped and reported as skipped**. No dataset, no GPU, no OpenCOOD checkout. What this tier can establish is bounded and worth stating plainly: **the committed results are internally consistent, and the documents still say what those results say.** It cannot establish that the results are reproducible from raw data.
* **Tier 2 --- full run**: `python tools/verify_results.py` runs all **35**. Additionally requires the sibling `../OpenCOOD/` checkout, the local grids `data/p2/p2_grid_*.csv` and the frozen models `data/p2/selector_B0*.pkl`. **Independent reproduction from the raw OPV2V data needs all three, and none of them is in this repository** --- so a third party can verify the internal chain here and must obtain the dataset and re-run steps 1--4 to verify the rest.
<!-- GATE-COUNTS:END -->

**Reproduction status, final wording (R62-3).** All **35** checks run and pass on the full experiment machine. A clean clone reproduces the content, comparison and generator tiers unaided; the artefact tier additionally requires `data/p2`, the frozen models, the sibling OpenCOOD checkout, PyTorch and Tectonic. The two-tier split describes what a clean clone can do, not a tier that went unrun here.
...
* **It does not affect the content tier.** All 19 content-tier checks run on the committed tree and
...
## Where2comm: which numbers are deterministic (V2-R36 A-2)

> "Where2comm thresholds were selected previously on Validate and remained frozen. The historical
> Validate sweep is retained only as a provisional tuning record and is excluded from all reported
> results. All reported Test and Culver-City results were regenerated using the deterministic
> evaluation pipeline."

Enforced by `tests/test_provisional_exclusion.py` (gate 30), which fails if a provisional validate
artefact is cited by `paper/main.tex`, `paper/supplementary.tex`, or read by any generator under
`tools/` or `projects/`.

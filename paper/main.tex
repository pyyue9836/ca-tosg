...
\begin{figure*}[t]
    ...
    \caption{Per-mode selection ratios versus SNR on OPV2V validate at $B_{\max}=0.20$, from the frozen grid. \emph{Top:} the frozen selector (solid) against \emph{that budget's $\lambda$-penalised oracle} (dashed) --- the oracle sees the frame BLER, \emph{not} the per-frame block outcome, so it is not clairvoyant. \emph{Bottom:} stacked-area view of the selector's action distribution over the deployed action set $\mathcal{S}=\{E,L,F\}$ under AWGN and Rayleigh (the dominated $C_{256}$ is a physical-layer comparator, not a deployed action, Section~\ref{sec:candidates}); the dotted line marks the measured $10$~dB policy knee. Two readings: under AWGN $\rho_F$ steps from $0$ to $0.472$ at $10$~dB and is flat above it, and under Rayleigh the selector's $\rho_E$ is $0.002$ (test) / $0.000$ (Culver-City) while the oracle spends $\rho_E=0.157$ / $0.133$ there --- the $E$-collapse limitation of Section~\ref{sec:pareto}. The three-budget version is Fig.~\ref{fig:decision_budgets}.}\label{fig:decision_ratio}
\end{figure*}

\subsection{Feature Importance}\label{sec:feat_imp}

Fig.~\ref{fig:feat_imp} and Table~\ref{tab:feat_imp} report the top Gini feature importances of the deployed selector. The two channel-side features dominate: the channel-type indicator $c_t$ contributes $34.2\%$ of importance and the estimated SNR $\hat\gamma_t$ a further $27.5\%$, so two of the $23$ inputs carry $61.7\%$ between them against $38.3\%$ spread over the $21$ perception-side cues. The strongest of those, \texttt{pcd\_mean\_range}, reaches only $3.0\%$.
Two cautions belong with that number. \emph{Importance is not sufficiency:} a Gini share says how often a feature is split on, not that the remaining $38.3\%$ is redundant, and the feature ablation shows the opposite --- with the channel features alone the selector stops requesting features at the tighter budgets and never recovers the full selector's operating point (Section~\ref{sec:ablation}). \emph{And the split is transport-specific:} it is measured under an LDPC + QAM cliff where the channel state is nearly decisive by construction; under graceful degradation the same cues carry the decision instead (Appendix~\ref{sec:jscc_aware}). That the channel-type indicator $c_t$ outranks the estimated SNR $\hat\gamma_t$ is consistent with the frame-level channel physics: under Rayleigh fading the feasible set collapses to $L$ regardless of SNR, so the channel-type flag is the single highest-information split for the decision.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig_feature_importance.pdf}
    \caption{Top-12 features by Gini importance of the frozen $B_{\max}=0.20$ selector (\texttt{data/p2/selector\_B020.pkl}, the pre-registered confirmatory operating point). The channel-side features (red) jointly account for $61.7\%$ of total importance; the $21$ perception-side LiDAR cues (blue) carry the remaining $38.3\%$ between them, none exceeding $3.0\%$ on its own. Importance is not sufficiency: see Section~\ref{sec:ablation}.}
    \label{fig:feat_imp}
\end{figure}
...
AWGN     &  12.0 & 0.517 & \textbf{0.8291} & 0.7297 \\
...
AWGN     &  10.0 & 0.567 & \textbf{0.8896} & 0.8187 \\
...
\begin{table}[t]
\centering
\caption{True end-to-end AP on the OPV2V \emph{Culver-City} split (real-road-layout domain shift, $T=550$ frames) under the \emph{frozen} validate-trained selector at $B_{\max}=0.20$, $200$ paired realisations per operating point. Same $10$~dB policy knee ($\rho_F$: $0\!\to\!0.160$), a $+1.8$ AP@$0.5$ point change across it. AWGN rows only: under Rayleigh the frozen selector requests $F$ at no SNR ($\rho_F=0$ throughout, Fig.~\ref{fig:decision_ratio}), so every Rayleigh row is identically the Fixed-$L$ value $0.7299$ / $0.6463$ and carries no information beyond it. Absolute AP is lower than on test because the domain is harder.}\label{tab:gen_true_e2e_culver}
\begin{tabular}{lcccc}
\toprule
Channel & SNR (dB) & $\rho_L$ & AP@0.5 & AP@0.7 \\
\midrule
...
AWGN     &  12.0 & 0.836 & \textbf{0.7476} & 0.6554 \\
...
\begin{figure}[t]
    ...
    $+0.0660$ ($95\%$ CI $[+0.0591,+0.0730]$).}
    \label{fig:difficulty}
\end{figure}

\subsection{Is a Learned Selector Necessary? Comparison with an SNR-Threshold Rule}\label{sec:threshold}

Since the channel features are the individually strongest cues (Section~\ref{sec:feat_imp}), we test
...
the codec degrades gracefully (Appendix~\ref{sec:jscc_aware}), and channel-side importance is not
...
using the task cues alone (Appendix~\ref{sec:jscc_aware}). The channel-averaged payloads are in Table~\ref{tab:headline_agg}. Against the re-tuned SNR-threshold rule on test at
...
\subsection{A Two-Parameter Hand Rule, and What It Costs}\label{sec:handrule}

A one-parameter SNR threshold is the first challenger to a learned selector; a hand rule with a
second gate is the next one, and we pre-registered it before running it. The rule adds an ego-side
difficulty gate to the channel gate: $a_t=E$ if $d_t\le\tau_E$, else $F$ if
$r_t=1-\mathrm{BLER}_F(\hat\gamma_t,c_t)\ge\tau_F$, else $L$, where $d_t$ is one of three difficulty
proxies drawn from the same $21$ committed cues, carried in both orientations, and both thresholds
are fitted on validate through the identical scene-level LOSO walk and the identical hard budget
constraint as the deployed selector.

\emph{At the two tighter budgets that rule cannot bid at all.} The walk selects never-$E$ and
never-$F$ at $B_{\max}=0.10$ and $0.20$---it degenerates to Fixed $L$---and at $B_{\max}=0.30$ the
freely fitted reliability gate rediscovers the SNR threshold almost exactly ($\rho_F=0.29853$ against
the threshold rule's $0.29883$ on validate). This is structural rather than a failure of fitting, and
it is the same four-rung ladder of Section~\ref{sec:ablation}: when the feature decision depends on
the channel alone, only four mean payloads are reachable, and $0.10$ and $0.20$ fall between rungs.

Because a comparator that cannot enter the comparison is not a comparator, we amended the arm---in
the record, before running it---so that the feature gate is a conjunction: request $F$ when the link
can carry it \emph{and} the frame is hard enough, $d_t\ge\tau_D$ and $r_t\ge\tau_F$. That is
\emph{three} scalars, not two, and we report it as three. It is feasible at every budget, and at the
primary cell it \emph{matches} the selector on F1: $0.89697$ against $0.89691$, a paired difference
of $+0.00005$ ($95\%$ CI $[-0.00002,+0.00012]$). We state that plainly rather than around it---at
this operating point a three-scalar hand rule is not worse in F1 than the learned selector---and no
further comparator was run after seeing it.

The difference lies on the other axis. The same hand rule spends $0.19900$~Msym against the
selector's $0.14141$, a paired difference of $+0.05760$ ($95\%$ CI $[+0.05663,+0.05853]$): $40.7\%$
more channel use for that $+0.00005$ F1, or equivalently the selector reaching the same F1 with
$28.9\%$ less channel. The mechanism is selectivity per transmitted frame. Over the cells each policy
actually sends $F$ on, the mean feature-over-object-level gain is $0.05022$ for the selector against
$0.03660$ for the hand rule and $0.03038$ for the SNR threshold---the last indistinguishable from the
$0.03042$ unconditional mean, i.e.\ the threshold rule's feature frames are no better chosen than
average. The selector buys more F1 per transmitted frame and therefore needs fewer of them; that is
the whole of the payload gap, and it is what the ego-side cues contribute on this transport.
...
$+0.0300$ F1 for $+0.1183$~Msym, and the third collaborator adds only $+0.0061$ more F1 for a further
$+0.0991$~Msym---about a fifth of the gain for three-quarters of the extra channel use. On test the
pattern is the same but flatter ($+0.0096$ then $+0.0008$), because that split is thinner and
...
noise up to $\sigma\approx1$~dB with $\le0.0002$ F1 loss; under a Jakes model at $60$~km/h the
SNR estimate decorrelates within ${\sim}10$~ms, costing $-0.0025$ F1 before plateauing; and
acting on a one-frame-stale decision costs at most $-0.0109$ F1 under our i.i.d.-per-frame
channel assumption (it changes $18.2\%$ of decisions). This near-immunity to SNR-estimation noise is consistent with the channel-type-anchored policy of Section~\ref{sec:feat_imp}: perturbing the continuous $\hat\gamma_t$ rarely flips a decision anchored on the discrete channel-type flag, and under Rayleigh the action remains $L$ regardless. Finally, the deployed

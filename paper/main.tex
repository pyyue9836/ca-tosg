...
Two findings stand out. First, neither half of the input is sufficient on its own, and the failure mode is the same in both directions: run through the identical pipeline under $B_{\max}=0.20$, the channel-only variant ($2$ features) and the cues-only variant ($21$ features) both collapse to always transmitting $L$---feature-request rate $\rho_F=0.000$, payload pinned at $B_L$, realised F1 at the fixed-$L$ floor $0.9011$---while the full selector reaches $0.9046$ at $0.095$~Msym. Cues without a channel signal cannot tell whether a feature request would survive the link; channel state without cues cannot tell whether it would be worth sending. Only the combination yields a policy that sits between the two extremes. These extra $F$ requests are channel-blind---the same $0.07\!\to\!0.16$ request share on every channel, since $\hat\gamma_t$ carries no channel type---so on features-infeasible channels, Rayleigh and OFDM below its ${\approx}24$~dB threshold, where the LDPC block almost always fails, they collapse to the ego floor. The channel-type flag $c_t$ restores the gating, the same root that makes it the model's top feature (Section~\ref{sec:feat_imp}). This does not contradict the sufficiency of the SNR threshold in Section~\ref{sec:threshold}: SNR is decisive as the \emph{threshold signal} that gates the feature request under AWGN, but as an \emph{additional RF feature} atop the perception cues it adds a noisy continuous dimension without an F1 gain---the two roles are distinct. Second, adding $c_t$ enables a channel-conditional policy---under Rayleigh the selector becomes $L$-dominant while under AWGN it activates $F$ above the LDPC threshold---so what the cues change is \emph{deployed channel use}, not accuracy: by selecting the low-payload $L$ action more often at equal F1, the full selector spends $0.240$~Msym versus $0.271$ for channel state alone.
...
Channel & SNR (dB) & $\rho_L$ & AP@0.5 & AP@0.7 \\
\midrule
AWGN     & 0.0  & 1.000 & 0.919 & 0.869 \\
AWGN     & 8.0  & 0.985 & 0.917 & 0.867 \\
AWGN     & 12.0 & 0.126 & 0.921 & 0.866 \\
AWGN     & 14.0 & 0.120 & 0.920 & 0.865 \\
AWGN     & 16.0 & 0.123 & \textbf{0.921} & \textbf{0.865} \\
AWGN     & 20.0 & 0.186 & 0.920 & 0.866 \\
Rayleigh & 0.0  & 1.000 & 0.919 & 0.869 \\
Rayleigh & 10.0 & 1.000 & 0.919 & 0.869 \\
Rayleigh & 20.0 & 1.000 & 0.919 & 0.869 \\
\bottomrule
\end{tabular}
\end{table}
...
Channel & SNR (dB) & $\rho_L$ & AP@0.5 & AP@0.7 \\
\midrule
AWGN     & 0.0  & 1.000 & 0.783 & 0.698 \\
AWGN     & 8.0  & 1.000 & 0.783 & 0.698 \\
AWGN     & 12.0 & 0.460 & 0.853 & 0.753 \\
AWGN     & 14.0 & 0.440 & 0.855 & 0.754 \\
AWGN     & 16.0 & 0.435 & \textbf{0.857} & \textbf{0.756} \\
AWGN     & 20.0 & 0.569 & 0.840 & 0.740 \\
Rayleigh & 0.0  & 1.000 & 0.783 & 0.698 \\
Rayleigh & 10.0 & 1.000 & 0.783 & 0.698 \\
Rayleigh & 20.0 & 1.000 & 0.783 & 0.698 \\
\bottomrule
\end{tabular}
\end{table}
...
test and $\mathbf{+0.106}$ ($95\%$ CI $[+0.097,+0.116]$) on the harder Culver-City
domain ($+0.035$ on validate), an order of magnitude above the all-channel mean, while
staying near zero on easy frames. The value of \method{} is therefore two-dimensional:
it materialises where a \emph{hard frame meets a reliable channel}, and the
channel-averaged headline necessarily dilutes it. (On easy frames under a good channel
the deployed selector occasionally over-requests $F$, a small $-0.0147$ F1 effect
that would be mitigated by a payload-penalised ($\lambda>0$) operating point on the
Lagrangian frontier of Section~\ref{sec:pareto}.)
...
using the task cues alone (Section~\ref{sec:jscc_aware}). Dropping the range, density, or object-count cue
...
\begin{figure}[t]
    ...
    deployment does not yet transfer this gain, Section~\ref{sec:jscc_aware}(b)). Error bars are
...
Section~\ref{sec:jscc_aware} reproduces under OFDM (in-distribution $+0.020$ F1 edge). Finally, the deployed

...
%
% The perception caches fused EVERY collaborator in a frame while the payload model charged ONE
% message (docs/experiment_protocol.md, Change-log P0). Ruling (a): the main experiment is the
% nearest single collaborator. Every absolute number below is WITHDRAWN pending the rewrite --
% F1, payload, the nine-cell table, the Pareto points, the ablations and the R9 margins.
%
%   corrected values so far ... docs/p0_corrigendum.md
%   pre-promotion state ...... git tag pre-p0-corrigendum
%   R9 survives the correction; its margins do not. Do not quote a retired number from this file.
%
% This is a source comment: it does NOT render. Nothing in the compiled PDF warns a reader, so the
% PDF should not be circulated until the rewrite lands.
% =====================================================================================
...
\quad \method{} (RF, frozen)              & \textbf{0.89148} & \textbf{0.0368} & $\mathbf{3.7\%}$ \\
...
\begin{figure*}[t]
    ...
    \caption{Realised effective F1 versus SNR under AWGN (left) and Rayleigh (right) on OPV2V validate at $B_{\max}=0.20$, every curve from the frozen grid. \method{} holds $L$ below the cliff---its own realised F1 there is $0.9071$, marginally above the Fixed-$L$ line at $0.9067$ because of the $E$/$L$ mix---and steps to $0.9243$ at the measured $10$~dB knee, reaching $0.9244$ by $20$~dB; the perfect-channel feature ceiling is $0.9193$ and the feasibility-masked oracle reaches $0.9294$, so the step recovers part of the headroom rather than all of it. Under Rayleigh the selector stays on Fixed $L$ throughout, flat at $0.9071$ across the whole range. The codec-comparison baselines are a prior-protocol arm and are in Appendix~\ref{sec:jscc_aware}.}\label{fig:ap_snr}
\end{figure*}
...
Fig.~\ref{fig:payload_snr} reports the average per-frame channel use of each policy against SNR, from the same frozen grid. At $B_{\max}=0.20$ \method{} holds the $L$-payload ($0.0238$~Msym/frame on validate---the $E$/$L$ mix, measured at AWGN $0$~dB and identical at every Rayleigh SNR) at every SNR under Rayleigh and for AWGN SNR $\le 8$~dB, then steps once at $10$~dB to $0.297$~Msym and stays there. The step is to about $30\%$ of the Fixed-$F$ payload, \emph{not} to a near-$F$ payload: even with the channel fully open the frozen selector requests the feature message on only about a quarter of frames. The fixed-mode baselines are flat at their constant payload and provide the worst-case bandwidth reference.
...
Fig.~\ref{fig:decision_ratio} shows the per-mode selection ratios $\rho_s$ against SNR, with the frozen selector overlaid on that budget's payload-penalised oracle and the stacked-area panel showing the share of each action of $\mathcal{S}=\{E,L,F\}$ at every SNR. Under AWGN the selector's adaptation knee is at $10$~dB, where the frame-error cliff clears: $\rho_F$ steps from $0$ to $0.283$ on validate ($0.256$ on test, $0.064$ on Culver-City) and is flat above it, with $\rho_L$ falling correspondingly. Under Rayleigh $\rho_L$ stays at essentially $1$ across the whole range. The panel also makes the $E$-collapse of Section~\ref{sec:pareto} visible: under Rayleigh the oracle spends a substantial share on the ego-only action---$\rho_E=0.157$ on test and $0.133$ on Culver-City---while the frozen selector's $\rho_E$ is $0.002$ and $0.000$. $E$ is a deployed action that the imitation objective almost never selects, which is a limitation of the trained policy rather than of the action set.
...
here. The comparison is therefore indicative of relative standing under ideal
transmission and is not on the same protocol as the frozen results. Under
...
channel assumption. This near-immunity to SNR-estimation noise is consistent with the channel-type-anchored policy of Section~\ref{sec:feat_imp}: perturbing the continuous $\hat\gamma_t$ rarely flips a decision anchored on the discrete channel-type flag, and under Rayleigh the action remains $L$ regardless. Finally, the deployed

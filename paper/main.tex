...
\begin{equation}
s_t \in \mathcal{S} = \{L, C_{16}\},
\label{eq:action_set}
\end{equation}
where $L$ denotes object-level communication and $C_{16}$ denotes the compressed feature-level mode under 16-QAM modulation with rate-$1/2$ LDPC coding. A second feature-level mode $C_{256}$ (256-QAM, same rate-$1/2$ LDPC) completes the granularity ladder addressable by the request field, but is dominated by $C_{16}$ and is therefore excluded from the deployed action set $\mathcal{S}$; we establish this exclusion in Section~\ref{sec:candidates}. The transmitted message is therefore
\begin{equation}
m_t = m_t^{s_t},
\end{equation}
...
\begin{table}[t]
\centering
\caption{Headline results: true end-to-end AP of \method{} across three OPV2V splits, in the fading/low-SNR fallback regime (selector holds $L$) and the AWGN $\hat\gamma_t\ge14$~dB feature-active regime (selector requests $C_{16}$). When the channel can carry features, \method{} lifts AP toward the feature-level ceiling on validate and Culver-City and remains comparable to object-level on the sparser test split; when the channel cannot, it safely retains the $L$ operating point. Payload is per-frame channel use (Msym, at rate-1/2); channel-\emph{averaged} \method{} payload is $0.158$--$0.251$~Msym/frame across splits, i.e.\ $16$--$25\%$ of Fixed $C_{16}$ ($0.990$).}
\label{tab:headline}
\begin{tabular}{llccc}
\toprule
...
oracle: on test, the deployed selector reaches F1 $0.909$ at $0.251$~Msym/frame,
within $0.005$ F1 of the oracle and at $25\%$ of the $C_{16}$ payload.

...
\begin{abstract}
Connected and automated vehicles can improve perception reliability by exchanging information through vehicle-to-vehicle (V2V) links. However, cooperative perception must operate under limited bandwidth and time-varying vehicular channels. Existing communication-efficient perception methods mainly reduce redundancy within a fixed semantic representation, such as object-level detections or feature-level messages. This paper shows that fixed semantic granularity is suboptimal: feature-level messages are potentially more informative but can become unreliable or wasteful under poor channel quality, while object-level messages are compact and stable but less expressive. We propose \emph{Channel-Aware Task-Oriented Semantic Granularity Selection} (\method), a receiver-driven framework in which the ego vehicle selects the communication granularity for each frame from its own perception cues and estimated channel state, and signals the choice to the collaborator via a $2$-bit request piggy-backed on the existing 802.11bd CAM frame. \method{} chooses one message type from object-level communication and compressed feature-level communication modes before any high-payload transmission. We derive the selector as the Lagrangian-relaxed solution of a constrained perception-communication problem and instantiate it with a $400$-tree Random Forest. Experiments on $1{,}980$ frames of the OPV2V dataset under AWGN and Rayleigh channels show that \method{} approaches the channel-aware oracle within $0.003$ realized frame F1 while reducing average payload by approximately $85\%$ compared with fixed feature-level transmission under LDPC + 16-QAM coding. The estimated SNR and channel-type features jointly account for $65\%$ of the selector's feature importance, dominating $21$ scene-side cues. The deployed selector incurs $52.8$~ms per frame on a single CPU core, fitting the $100$~ms budget of a 10~Hz LiDAR cycle. These results demonstrate that channel-aware semantic granularity adaptation is an effective strategy for bandwidth-constrained V2V cooperative perception.
\end{abstract}
...
Motivated by this observation, this paper proposes \emph{Channel-Aware Task-Oriented Semantic Granularity Selection} (\method) for bandwidth-constrained V2V cooperative perception. Instead of transmitting a fixed representation for every frame, \method{} selects one message type from a set of semantic communication candidates. In the considered implementation, the selector chooses among an object-level message and compressed feature-level messages with different channel-use characteristics. The decision is made using sender-side task cues, such as prediction confidence, visibility, temporal stability, and detection uncertainty, together with estimated channel quality such as SNR. The selected message is then transmitted through the V2V link and fused with the ego vehicle's local feature for 3D object detection. An overview of the resulting framework is shown in Fig.~\ref{fig:overview}.
...
\begin{itemize}
    ...
    \item We propose \method, a lightweight Random Forest selector~\cite{breiman2001random} that uses online perception cues and estimated SNR to choose among compact object-level communication and compressed feature-level communication modes.
    ...
    \item Experiments on OPV2V show that \method{} avoids unreliable high-payload feature transmission under poor channel conditions and approaches oracle performance when the channel supports richer communication. We characterise it as \emph{Pareto-optimal} on the payload--F1 plane rather than merely better on average, show the gain concentrates on hard frames under usable channels (up to $+0.045$ F1), and verify cross-split and Culver-City domain-shift generalisation with a frozen selector.
    \item We report an honest analysis of selector design: feature ablations, an SNR-threshold baseline that matches the learned selector on F1 (so the contribution is a \emph{lightweight, bandwidth-efficient, interpretable channel-aware policy}, not a complex learner), model and latency comparisons, Rician/CSI-aging robustness, and a negative finding that 256-QAM is dominated under LDPC 1/2.
\end{itemize}
...
V2V cooperative perception aims to improve the perception capability of an ego vehicle by exploiting information from surrounding connected vehicles~\cite{xu2022opv2v}. Compared with single-agent perception, cooperative perception can provide complementary viewpoints and alleviate occlusion, long-range sparsity, and limited field-of-view problems. Existing methods are commonly categorized according to the stage at which information is shared: early fusion, intermediate fusion, and late fusion.
...
However, most communication-efficient cooperative perception methods still operate within a fixed semantic level. They usually assume that the system will transmit feature-level information and then optimize which feature tokens or compressed representations should be sent. In contrast, our work studies a higher-level decision: whether the current frame should use feature-level communication at all. The proposed \method{} framework can select compact object-level communication when feature-level information is unnecessary or unreliable, and activate feature-level communication only when its expected task benefit justifies the additional payload. The granularity selection of \method{} is therefore orthogonal to the spatial selection of Where2comm or the codec design of importance-map JSCC~\cite{sheng2024importance}; either can be used inside the feature-level branch of our framework.

\subsection{Semantic and Channel-Aware Communication for Vehicular Perception}

Semantic communication has recently attracted attention as a task-oriented alternative to conventional bit-level communication~\cite{sheng2024importance,gan2026scomcp}. Instead of minimizing reconstruction error or bit error rate alone, semantic communication optimizes the transmitted representation for downstream task performance. In cooperative perception, this means that the communication module should preserve information useful for detection, rather than reconstructing the full feature map perfectly.
...
\begin{figure}[t]
    ...
    \caption{Block-error rate of the feature-level message under LDPC $1/2$ with 16-QAM and 256-QAM, over AWGN (solid) and Rayleigh (dashed) channels. The 16-QAM AWGN cliff at $\approx 12$~dB defines the selector knee (shaded); the 256-QAM cliff sits far to its right, so by the time 256-QAM is reliable the 16-QAM block is already error-free---this is the basis of the 256-QAM dominance discussed in Section~\ref{sec:negative}. Under Rayleigh fading both curves degrade so slowly that feature-level transmission never becomes reliable in $0$--$20$~dB, explaining the selector's conservative $L$-only behaviour.}
    \label{fig:bler}
\end{figure}
...
\begin{equation}
B_L = 0.024, \quad B_{C_{16}} = 1.98/4 \approx 0.495, \quad B_{C_{256}} = 1.98/8 \approx 0.248,
\end{equation}
all in Mbit/frame, where the divisors $4$ and $8$ are the bits-per-symbol of 16-QAM and 256-QAM, respectively. The average communication cost over $T$ evaluated frames is
\begin{equation}
...
The goal of channel-aware semantic granularity selection is to choose a policy that balances detection utility and communication cost. Let $z_t$ denote the online task cue vector extracted from the sender-side perception stream, and let $\gamma_t$ denote the estimated channel quality. The selector is defined as
\begin{equation}
...
\begin{table}[t]
\centering
\caption{Ego-side online task cues used by the \method{} selector ($d=21$). All cues are computed locally on the ego vehicle before message selection, removing any causality issue with sender-side cue dependence.}
\label{tab:cues}
\begin{tabular}{lll}
\toprule
...
Table~\ref{tab:headline} summarises the mean realised F1 and average payload for the proposed selector against fixed strategies and the channel-aware oracle, averaged across SNRs $0$--$20$~dB and over an equal mix of AWGN and Rayleigh channel realisations.

\begin{table}[t]
\centering
\caption{Headline results on OPV2V validate ($T=1{,}980$ frames), mean over SNR $\in[0,20]$~dB with $50/50$ AWGN/Rayleigh mix. The proposed selector recovers $99.7\%$ of the oracle's F1 with less than $16\%$ of the bandwidth of Fixed $C_{16}$.}
\label{tab:headline}
\begin{tabular}{lcc}
\toprule
Policy & Payload (Mbit/frame) & Mean F1 \\
\midrule
Channel-aware oracle           & 0.074 & \textbf{0.868} \\
Fixed $L$ (PointPillar\_Late)   & 0.024 & 0.862 \\
Fixed $C_{16}$ (LDPC + 16-QAM)  & 0.495 & 0.385 \\
Fixed $C_{256}$ (LDPC + 256-QAM)& 0.248 & 0.101 \\
\midrule
\method{} (ours, RF + CSI + ch) & \textbf{0.076} & \textbf{0.865} \\
\bottomrule
\end{tabular}
\end{table}

Three observations are immediate. (i) Both fixed feature-level policies are \emph{strictly dominated by Fixed $L$}: they use $10\text{--}21\times$ more bandwidth and produce lower realised F1 because of the LDPC cliff at low SNR. (ii) The channel-aware oracle achieves only marginally higher F1 than Fixed $L$ ($0.868$ vs.\ $0.862$), confirming that the value of feature-level transmission is realised only on a small subset of frames under realistic channels. (iii) \method{} attains $99.7\%$ of the oracle's F1 ($0.865$ vs.\ $0.868$) with $0.076$~Mbit/frame, i.e., $84.6\%$ less bandwidth than Fixed $C_{16}$.
...
Fig.~\ref{fig:ap_snr} reports AP@0.5 and AP@0.7 under AWGN and Rayleigh, comparing \method{} with fixed-mode baselines, ImportanceMapJSCC~\cite{sheng2024importance}, LDPC + 16/256-QAM separate coding, the channel-aware oracle, and the perfect-channel upper bound.
...
\begin{figure*}[t]
    ...
    \caption{Detection performance versus SNR under AWGN and Rayleigh channels. \method{} (black star) approaches the channel-aware oracle (magenta diamond) under AWGN by activating $C_{16}$ when the channel can support it. Under Rayleigh fading, the selector converges to a conservative $L$-dominant policy, matching Fixed $L$ closely; the residual gap to the JSCC-aware oracle is discussed in Section~\ref{sec:discussion}. LDPC + QAM separate-coding baselines fall well below Fixed $L$ across all SNRs, demonstrating that traditional digital feature-level transmission is strictly dominated by object-level communication in this regime.}
    \label{fig:ap_snr}
\end{figure*}

Under AWGN, \method{} sits on the Fixed $L$ line for SNR $\le 10$~dB and rises to approximately $0.86$ AP@0.5 for SNR $\ge 14$~dB, tracking ImportanceMapJSCC and the channel-aware oracle within $0.01$~AP. The bandwidth panel (Section~\ref{sec:payload_snr}) reflects this transition with a step-shaped payload increase at the LDPC decoding threshold. Under Rayleigh, \method{} remains near Fixed $L$ across the entire SNR range; we attribute this to the residual deep-fade risk that keeps the channel-aware oracle from confidently selecting $C_{16}$ even at $20$~dB mean SNR. This is consistent with the empirical observation that fixed LDPC baselines never recover above the ego-only AP floor of $\approx 0.63$ under Rayleigh.

\subsection{Frame-Level F1 versus SNR}

Fig.~\ref{fig:f1_snr} shows the mean realised frame F1 under the analytical block-loss model of Eq.~\eqref{eq:eff_C}, which underpins the oracle and the selector's training label. Fixed feature-level communication collapses to near-zero F1 below $10$~dB (AWGN) and across the entire range (Rayleigh) because of the BLER cliff; \method{} stays at or above Fixed $L$ throughout.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{fig_snr_vs_f1.pdf}\hfill
    \includegraphics[width=0.48\textwidth]{fig_f1_rayleigh.pdf}
    \caption{Mean realised frame F1 versus SNR under AWGN (left, v1 results) and Rayleigh (right, v2 results). \method{} remains close to the channel-aware oracle and avoids the cliff of fixed feature-level communication at low SNR.}
    \label{fig:f1_snr}
\end{figure*}
...
\begin{table}[t]
\centering
\caption{Top-7 features by Gini importance for the deployed \method{} selector. The two channel-side features account for $65\%$ of total importance.}
\label{tab:feat_imp}
\begin{tabular}{lc}
\toprule
...
\subsection{CSI Estimation Noise Robustness}\label{sec:csi_noise}

In any deployed V2X stack the SNR estimate $\hat\gamma_t$ is itself noisy. To
quantify the selector's robustness to this noise we keep the true channel SNR
fixed at $\gamma=10$~dB but corrupt the selector's input $\hat\gamma_t$ with
zero-mean Gaussian noise of standard deviation $\sigma \in
\{0,0.5,1,2,3,5\}$~dB and average each operating point over $10$ noise draws.
Results are reported in Fig.~\ref{fig:csi_noise}.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig_csi_noise.pdf}
    \caption{Mean realised frame F1 versus SNR-estimation noise $\sigma$ at fixed true SNR $\gamma=10$~dB, averaged over $10$ noise realisations per point. The deployed selector is stable for $\sigma \le 1$~dB under AWGN and remains stable across the entire range under Rayleigh, because the Rayleigh policy is already saturated at $L$.}
    \label{fig:csi_noise}
\end{figure}

Under AWGN, the selector tolerates SNR-estimation noise up to
$\sigma\approx 1$~dB with $\le 0.003$~F1 degradation; F1 degrades by
$0.025$ at $\sigma=2$~dB and by $0.070$ at $\sigma=5$~dB as the selector
begins to mis-classify low-SNR frames as feature-level-eligible and pays
a payload penalty without a corresponding F1 gain. Under Rayleigh, the
deployed policy is saturated at $L$ across the entire SNR range
(Section~\ref{sec:decision_ratio}), so the F1 is insensitive to the SNR
estimate. This asymmetry has a clean operational interpretation: the
selector needs a precise SNR estimate \emph{only} in the regime where
feature-level communication is a viable option; in regimes where the
oracle would refuse feature-level transmission regardless, CSI accuracy
is irrelevant.
...
\begin{table}[t]
\centering
\caption{True end-to-end AP under the deployed selector, computed by concatenating cached per-frame predictions according to the selector's choice and scoring against OPV2V ground truth at $5$ stochastic channel realisations per operating point. The AP@$0.5$ knee at AWGN $\ge 12$~dB matches the policy transition observed in Fig.~\ref{fig:decision_ratio}; under Rayleigh the selector stays at the Fixed-$L$ point across the full $0$--$20$~dB range, consistent with the deep-fade regime.}\label{tab:true_e2e}
\begin{tabular}{lcccc}
\toprule
Channel & SNR (dB) & $\rho_L$ & AP@0.5 & AP@0.7 \\
\midrule
AWGN     & 0.0  & 1.000 & 0.838 & 0.757 \\
AWGN     & 8.0  & 1.000 & 0.838 & 0.757 \\
AWGN     & 12.0 & 0.909 & 0.844 & 0.769 \\
AWGN     & 14.0 & 0.331 & \textbf{0.863} & \textbf{0.791} \\
AWGN     & 16.0 & 0.326 & 0.863 & 0.790 \\
AWGN     & 20.0 & 0.362 & 0.863 & 0.791 \\
Rayleigh & 0.0  & 1.000 & 0.838 & 0.757 \\
Rayleigh & 10.0 & 1.000 & 0.838 & 0.757 \\
Rayleigh & 20.0 & 0.998 & 0.838 & 0.757 \\
\bottomrule
\end{tabular}
\end{table}

Two observations support the central claim. First, the deployment-mode AP at AWGN $14$~dB reaches $0.863$ at IoU $0.5$ and $0.791$ at IoU $0.7$, sitting within $0.007$ AP of the perfect-channel reference of the underlying attentive-compression model ($0.870$, see Section~\ref{sec:headline}). The selector therefore captures essentially all of the benefit of feature-level transmission once the LDPC threshold is crossed, while paying only $0.339$~Mbit/frame instead of $0.495$. Second, the Rayleigh AP curve is flat across the full SNR range and matches Fixed $L$, confirming that the selector's conservative fading-regime behaviour, identified in Section~\ref{sec:headline}, is not a frame-F1 artefact but a genuine end-to-end property.
...
Tables~\ref{tab:gen_true_e2e} and~\ref{tab:gen_true_e2e_culver} report the true end-to-end AP on the test and Culver-City splits respectively. The qualitative behaviour matches validate exactly on both. On test, the selector remains at Fixed $L$ up to $8$~dB, begins activating $C_{16}$ at $12$~dB ($\rho_L=0.95$), and crosses to a feature-dominant policy at $14$~dB ($\rho_L=0.33$), lifting AP@$0.5$ from $0.859$ to $0.882$ and AP@$0.7$ from $0.781$ to $0.797$---within $0.017$ AP@$0.5$ of the test-split perfect-channel reference of the attentive-compression model ($0.899$). On Culver-City the same knee appears at $12$--$14$~dB ($\rho_L: 0.97 \to 0.20$), lifting AP@$0.5$ from $0.799$ to $0.853$ ($+5.4$ points) and AP@$0.7$ from $0.668$ to $0.729$ ($+6.1$ points); the \emph{relative} adaptation gain is in fact larger on the harder domain, consistent with the wider per-frame margin noted above. Under Rayleigh the selector stays at the Fixed-$L$ operating point across the entire $0$--$20$~dB range on both splits, because the deep-fade BLER never falls low enough to make feature-level transmission worthwhile. The channel-adaptive AP gain---the actual value proposition of \method{}---therefore reproduces both on a scene-disjoint split and across a real-road-layout domain shift, with a frozen selector.

\begin{table}[t]
\centering
\caption{True end-to-end AP on the OPV2V \emph{test} split under the validate-trained selector, $5$ stochastic channel realisations per operating point. The AWGN AP@$0.5$ knee at $\ge 12$~dB and the flat Rayleigh curve at the Fixed-$L$ point both reproduce the validate behaviour of Table~\ref{tab:true_e2e}.}\label{tab:gen_true_e2e}
\begin{tabular}{lcccc}
\toprule
Channel & SNR (dB) & $\rho_L$ & AP@0.5 & AP@0.7 \\
\midrule
AWGN     & 0.0  & 1.000 & 0.859 & 0.781 \\
AWGN     & 8.0  & 1.000 & 0.859 & 0.781 \\
AWGN     & 12.0 & 0.953 & 0.865 & 0.789 \\
AWGN     & 14.0 & 0.329 & 0.881 & 0.795 \\
AWGN     & 16.0 & 0.329 & \textbf{0.882} & 0.796 \\
AWGN     & 20.0 & 0.532 & 0.881 & \textbf{0.797} \\
Rayleigh & 0.0  & 1.000 & 0.859 & 0.781 \\
Rayleigh & 10.0 & 1.000 & 0.859 & 0.781 \\
Rayleigh & 20.0 & 1.000 & 0.859 & 0.781 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{True end-to-end AP on the OPV2V \emph{Culver-City} split (real-road-layout domain shift, $T=550$ frames) under the validate-trained selector, $5$ stochastic channel realisations per operating point. The AWGN knee at $12$--$14$~dB and the flat Rayleigh curve reproduce the behaviour of Tables~\ref{tab:true_e2e} and~\ref{tab:gen_true_e2e}; absolute AP is lower than on test because the domain is harder, but the relative adaptation gain is larger.}\label{tab:gen_true_e2e_culver}
\begin{tabular}{lcccc}
\toprule
Channel & SNR (dB) & $\rho_L$ & AP@0.5 & AP@0.7 \\
\midrule
AWGN     & 0.0  & 1.000 & 0.799 & 0.668 \\
AWGN     & 8.0  & 1.000 & 0.799 & 0.668 \\
AWGN     & 12.0 & 0.965 & 0.799 & 0.669 \\
AWGN     & 14.0 & 0.198 & 0.852 & \textbf{0.729} \\
AWGN     & 16.0 & 0.200 & \textbf{0.853} & 0.729 \\
AWGN     & 20.0 & 0.344 & 0.846 & 0.721 \\
Rayleigh & 0.0  & 1.000 & 0.799 & 0.668 \\
Rayleigh & 10.0 & 1.000 & 0.799 & 0.668 \\
Rayleigh & 20.0 & 1.000 & 0.799 & 0.668 \\
\bottomrule
\end{tabular}
\end{table}
...
($+0.000/+0.008$ easy/hard on test); but on realisations where the channel can carry
features (AWGN, SNR $\ge 14$~dB) the gain on hard frames reaches $\mathbf{+0.045}$ on
test ($+0.039$ on validate), an order of magnitude above the all-channel mean, while
...
the deployed selector occasionally over-requests $C_{16}$, a small $-0.013$ F1 effect
...
\begin{figure}[t]
    ...
    AWGN, SNR$\ge$14~dB---when the channel can support features the hard-frame gain over
    Fixed $L$ reaches $+0.045$. Numbers above bars are CA-TOSG$-$Fixed-$L$.}
...
(train on validate, evaluate on test). We report this honestly: \emph{on realised F1
the RF does not beat the SNR-threshold rule}---the best threshold reaches F1 $0.890$
and the full RF $0.887$. The RF's advantage is \emph{bandwidth efficiency}: it attains
the same F1 at $0.028$~Mbit/frame versus the threshold's $0.072$--$0.117$, because it
uses perception cues to send features only on frames that benefit, whereas the
threshold floods every high-SNR frame. We therefore frame \method{} as a
\emph{lightweight channel-aware selection policy} whose value is Pareto/bandwidth
efficiency, not raw accuracy; the threshold rule is a fully interpretable special case.
Dropping the range, density, or object-count cue groups changes F1 by $<0.001$.

\begin{table}[t]
\centering
\caption{Selector feature ablation and SNR-threshold rule (train: validate, eval: test).
Realised F1 is nearly invariant across selectors; they differ in payload. The
SNR-threshold rule matches the RF on F1 at higher payload.}\label{tab:ablation_threshold}
\begin{tabular}{lccc}
\toprule
Selector & \#feat & Realised F1 & Payload \\
\midrule
Channel only ($\hat\gamma,c$)        & 2  & 0.888 & 0.066 \\
Perception cues only                 & 21 & 0.887 & 0.024 \\
Perception $+\ \hat\gamma$           & 22 & 0.887 & 0.024 \\
Full (all features)                  & 23 & 0.887 & \textbf{0.028} \\
SNR-threshold $\tau{=}12$            & -- & \textbf{0.890} & 0.117 \\
SNR-threshold $\tau{=}16$            & -- & 0.889 & 0.072 \\
\midrule
Oracle / Fixed $L$                   & -- & 0.894 / 0.887 & 0.074 / 0.024 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Selector Model Comparison}\label{sec:models}

Table~\ref{tab:models} compares candidate selectors at batch size $1$ (the $10$~Hz
operating point; latencies on the host of Section~\ref{sec:latency}). All models reach
the same realised F1 ($0.887$--$0.889$); they trade payload, latency, and size. A
decision tree or logistic-regression selector matches the random forest's F1 at
$>10\times$ lower latency and $\sim100\times$ smaller size, while the random forest is
the most payload-efficient. This confirms that model complexity is not where the
performance comes from, and that a far lighter selector is deployable if latency is
critical.

\begin{table}[t]
\centering
\caption{Selector models (train: validate, eval: test). Latency is per-frame at batch
size 1 on the experimental host; F1 is essentially model-independent.}\label{tab:models}
\begin{tabular}{lcccc}
\toprule
Model & Realised F1 & Payload & Latency (ms) & Size \\
\midrule
Decision Tree          & 0.889 & 0.072 & 0.40 & 11~KB \\
Logistic Regression    & 0.889 & 0.062 & 0.55 & 2~KB \\
RBF-SVM                & 0.888 & 0.041 & 0.57 & 82~KB \\
Small MLP (32,16)      & 0.889 & 0.057 & 0.55 & 55~KB \\
Random Forest (400)    & 0.887 & \textbf{0.028} & 8.3 & 3.5~MB \\
SNR-threshold ($\tau{=}16$) & 0.889 & 0.072 & $\sim0$ & -- \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Scene-Content Subsets and Object-Level Reliability}\label{sec:subsets}

\textbf{Scene subsets.} To check the selector activates features for the right reasons,
we form top-tercile subsets by cached cues. On occlusion-proxy frames (high
object-level false-positive count) the selector's feature-activation rate rises to
$0.17$ versus $0.10$ on the complement and its good-channel gain to $+0.031$ vs
$+0.013$ (validate); the sparse-LiDAR subset behaves similarly on test
($+0.033$ vs $+0.001$). The long-range subset is inconclusive---our cue is the count
of far LiDAR \emph{points}, a poor proxy for far \emph{objects} (which OPV2V does not
label per-frame), and the selector does not favour features there. We report this
rather than select a favourable proxy.

\textbf{Object-level reliability.} The main results treat $L$ as perfectly reliable.
Relaxing this---a lost $L$ falls back to the ego-only floor ($0.63$), with $L$
block-error rate up to $10\%$ and repetition cost up to $3\times$---the
CA-TOSG-over-$L$ gain \emph{grows} monotonically from $+0.002$ to $+0.005$ as $L$
degrades, and the conclusion (CA-TOSG $\ge$ Fixed $L$) is unchanged: the result does
not rest on an over-strong reliability assumption for $L$.

\subsection{Vehicular Channels: Rician Fading and CSI Aging}\label{sec:vehicular}

\textbf{Rician fading.} We extend the channel model to Rician fading (BLER obtained by
averaging the AWGN BLER over the Rician SNR density, $K$-factor sweeping
$0=$Rayleigh$\to\infty=$AWGN). Fig.~\ref{fig:rician} shows the selector's
feature-activation knee moving smoothly from Rayleigh-like (stay at $L$) to AWGN-like
(activate $C_{16}$) as the line-of-sight component strengthens---the framework spans
the full fading-severity range without modification.

\textbf{CSI aging.} Under a Jakes model at $5.9$~GHz and $60$~km/h (Doppler
$\approx328$~Hz), the SNR estimate decorrelates within $\sim10$~ms, so a stale CSI
estimate degrades realised F1 from $0.888$ to $\approx0.873$ ($-1.5\%$) and then
plateaus---the loss is bounded because the selector falls back to the robust $L$ when
CSI is unreliable. This complements the CSI-noise ablation of Section~\ref{sec:csi_noise}.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig_rician.pdf}
    \caption{Feature-level activation rate of the channel-aware oracle versus mean SNR
    under Rician fading of increasing $K$-factor, between the Rayleigh and AWGN limits.}
    \label{fig:rician}
\end{figure}

\subsection{Request Latency and Decision Causality}\label{sec:causality}

The ego decides on its own frame-$t$ cues and CSI and signals a $2$-bit request, so
sub-frame request delays do not change the decided frame. The residual risk is
whole-frame staleness: at $10$~Hz a one-frame round trip applies the frame-$t$ decision
to frame $t{+}1$. The oracle action changes between consecutive frames $18\%$ of the
time at a $100$~ms delay; acting on a one-frame-stale decision costs up to
$-0.057$ realised F1, but this is a pessimistic bound under the i.i.d.-per-frame channel
assumption of our protocol (a one-frame-old \emph{channel} decision is near-random,
consistent with the $\sim10$~ms decorrelation above). Because CSI and cues are computed
locally at decision time, the architecture avoids most of this; a persistent per-round
mode preference bounds the remainder.

\subsection{JSCC-Aware Granularity Selection}\label{sec:jscc_aware}

If the feature branch used importance-map JSCC (graceful degradation) instead of
LDPC+QAM (cliff), is selection still needed? Fig.~\ref{fig:jscc_aware} plots AP@$0.5$
versus SNR from our ImportanceMapJSCC reproduction: JSCC degrades gracefully and stays
above the LDPC curves at low SNR, though still marginally below Fixed $L$ at
$0$~dB ($0.833$ vs $0.840$). Graceful JSCC weakens the \emph{channel}-driven case for
selection, but the \emph{task}-driven case remains: on easy frames object-level $L$
already matches feature-level (Section~\ref{sec:difficulty}), so a
$\{L,\text{Feature-JSCC}\}$ selector still saves bandwidth by not transmitting features
when the frame does not need them. A per-frame JSCC-aware selector requires per-frame
JSCC predictions, which our cached sweep provides only in aggregate; we specify it as
immediate future work (re-cache per-frame JSCC F1, relabel the oracle, retrain the
selector---no backbone change).

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig_jscc_aware.pdf}
    \caption{Feature-branch coding under AWGN: importance-map JSCC (graceful) versus
    LDPC+16/256-QAM (cliff), with the perfect-channel upper bound, ego-only floor, and
    channel-invariant Fixed $L$.}
    \label{fig:jscc_aware}
\end{figure}
...
using a checkpoint trained for $37$ epochs in our reproduction.
Under perfect channel (lossless feature-level transmission), Where2comm
achieves AP@0.5 = $0.809$ and AP@0.7 = $0.672$. These numbers sit below
the ImportanceMapJSCC perfect-channel reference of $0.866 / 0.728$ and
below the channel-invariant Fixed $L$ value of $0.840 / 0.760$. We
attribute this gap to the under-training of our Where2comm
reproduction; the original work~\cite{hu2022where2comm} requires
considerably longer training schedules than were available in our
single-GPU setting. Importantly, the qualitative finding is unaffected:
Where2comm is a spatial-mask method that operates inside a single
semantic granularity, and is therefore complementary rather than
competing with \method{}; a future extension can deploy Where2comm
inside the $C_q$ branches of \method{} while letting the granularity
selector decide when feature-level communication is activated.

\subsection{Inference Latency}\label{sec:latency}

To validate deployability we measure the per-frame inference latency of the deployed selector on a single CPU core (Intel i7 host of the experimental setup). Random Forest \texttt{predict()} is called on per-frame feature vectors with the cache warmed by $100$ priming calls and then measured over $2{,}000$ trials. At batch size $1$, the operating point relevant for a $10$~Hz online deployment, the mean per-frame latency is $\boldsymbol{52.8 \pm 5.7}$~ms with median $54.7$~ms, $95$th-percentile $59.1$~ms, and $99$th-percentile $69.5$~ms. The full $\mathrm{P99}$ latency therefore sits at $69$~ms, leaving a $31$~ms margin within the $100$~ms cycle of a $10$~Hz LiDAR sensor. At batch size $100$ (an offline batch-evaluation operating point), per-sample latency drops to $0.47 \pm 0.03$~ms, indicating that the bulk of the per-call cost at batch $1$ is in tree-traversal dispatch rather than per-feature computation; ahead-of-time compilation of the tree ensemble (e.g., via ONNX) would close this gap if a stricter latency budget were required, but is not necessary at $10$~Hz.

\subsection{Design Ablation: Action-Space Cliff Matching}\label{sec:negative}

An empirical finding from the $3$-way action set $\mathcal{S}=\{L,C_{16},C_{256}\}$ is that the channel-aware oracle never selects the $C_{256}$ action: $\rho_{C_{256}}^\star = 0.0\%$ across both channels and all SNRs in the evaluated range. We interpret this as an intentional ablation of the action-space design: $C_{16}$ and $C_{256}$ as defined here share the \emph{same} LDPC rate $1/2$ but trade modulation order against BLER, and the resulting cliffs are not rate-matched. Under LDPC 1/2 with AWGN, $\mathrm{BLER}_{256}(\gamma) > 0.5$ for all $\gamma \le 16$~dB, whereas $\mathrm{BLER}_{16}(\gamma) < 0.05$ already at $\gamma = 12$~dB (Fig.~\ref{fig:bler}); by the time 256-QAM is reliable, 16-QAM is already saturated, and Eq.~\eqref{eq:lagrangian_oracle} prefers the latter at every SNR.

The methodological implication is constructive. A truly useful third action would have to fall between $C_{16}$ and $L$ on \emph{both} the rate and the cliff-SNR axes simultaneously, which the LDPC 1/2 + 256-QAM pair fails to satisfy. A natural rate-matched extension that we leave to follow-up work is a $C_{256}^{3/4}$ branch combining 256-QAM with a higher LDPC rate (e.g., $3/4$ or $5/6$); the rate increase compensates for the modulation order's higher cliff, and the resulting branch should be non-dominated above approximately $18$~dB AWGN. Our framework accommodates this extension without retraining the perception backbone: only the BLER table and the action set need updating, and the selector retrains in under one minute on the existing $1{,}980$-frame dataset.

\subsection{Discussion and Limitations}\label{sec:discussion}

Three limitations of the present study are worth flagging explicitly.

\textbf{Conservative block-loss model.} The effective-F1 model of Eq.~\eqref{eq:eff_C} sets a frame's contribution to zero whenever an LDPC block error occurs. Empirically, a receiver that detects block loss can fall back to ego-only inference and recover the ego AP floor of approximately $0.63$. Replacing the multiplicative model with a soft floor of the form $F_t^{C_q} = (1-\mathrm{BLER}_q) F_t^{C_q,\mathrm{clean}} + \mathrm{BLER}_q \cdot F_t^{\mathrm{ego}}$ would close part of the gap between the fixed-$C$ curves and Fixed $L$ in Fig.~\ref{fig:ap_snr}, but would not alter the dominance ordering. We adopt the multiplicative model because it makes the selector's training labels conservative, and we report the JSCC baseline of~\cite{sheng2024importance} alongside the analytical curves to anchor the absolute scale. The end-to-end verification in Section~\ref{sec:e2e} (Table~\ref{tab:e2e}) confirms that this analytical bound is in fact conservative: the deployed selector achieves equal or higher F1 under the empirical floor than under the multiplicative model.

\textbf{LDPC-trained selector vs.\ JSCC baseline.} The deployed selector is trained against the LDPC + QAM effective-F1 model. When compared against the ImportanceMapJSCC baseline in Fig.~\ref{fig:ap_snr}, the selector is therefore more conservative than necessary in the Rayleigh regime, because JSCC degrades gracefully rather than cliffing. Retraining the selector with the JSCC effective-F1 model would tighten the gap to the JSCC-aware oracle (the magenta line) under Rayleigh. We treat this as a methodologically clean direction for follow-up work rather than a re-tuning of the present results.

\textbf{Cross-dataset evaluation.} The generalisation study of Section~\ref{sec:generalisation} establishes that the validate-trained selector transfers without retraining to the scene-disjoint OPV2V test split ($T=2{,}170$ frames) and to the Culver-City domain shift ($T=550$ frames, real road layout), reproducing the headline payload--F1 frontier and the channel-adaptive end-to-end AP behaviour on both. We have not, however, evaluated second-party V2V perception datasets such as V2X-Sim or V2V4Real, which remain the highest-priority follow-up. The selector's input cues are dataset-agnostic, the BLER tables are independent of the perception backbone, and only the per-frame predictions need to be re-cached on a new dataset; we therefore expect the qualitative findings (LDPC + QAM dominance by Fixed $L$, $\hat\gamma_t$ + $c_t$ accounting for the bulk of feature importance, AWGN-side adaptation knee at the LDPC threshold) to transfer, while the absolute payload--F1 frontier may shift with the underlying perception model's clean-channel performance.

\textbf{Multi-vehicle shared bandwidth.} This work studies a single ego--collaborator
link. A practical V2V deployment has several collaborators contending for a shared
bandwidth budget, turning per-frame granularity selection into a joint
selection-and-scheduling problem (which collaborators transmit, at which granularity,
under a total payload cap). Our per-link selector is the natural per-CAV primitive for
such a scheduler; the joint multi-CAV problem---comparing all-$L$, all-$C$, top-$k$
distance/confidence selection, per-CAV \method{}, and a bandwidth-constrained
scheduler on ego AP versus total payload and over-budget rate---is a substantial system
extension we leave to future work rather than approximate here.

Other future work includes (i) evaluation on second-party datasets (V2X-Sim, V2V4Real)
via the frozen-selector transfer protocol above; (ii) a per-frame JSCC-aware selector
(Section~\ref{sec:jscc_aware}); (iii) a rate-matched third action
($C_{256}^{3/4}$, Section~\ref{sec:negative}); (iv) a Where2comm-style spatial
confidence module inside the $C_q$ branches~\cite{hu2022where2comm}; and (v)
integration with spread-spectrum signalling for jamming resilience in safety-critical
V2X scenarios.

\section{Conclusion}

We presented \method{}, a channel-aware task-oriented semantic granularity selector for V2V cooperative perception. The selector takes per-frame LiDAR-derived perception cues together with an estimated SNR and a channel-type indicator and outputs one of three communication modes: a compact object-level message $L$, or a compressed feature-level message under 16- or 256-QAM coding. The selector is a lightweight Random Forest that adds no learnable parameters to the perception pipeline; it runs in $52.8 \pm 5.7$~ms ($\mathrm{P95} = 59.1$~ms) per frame on a single CPU core, comfortably within the $100$~ms budget of a $10$~Hz LiDAR cycle without any framework-level optimisation.

Experiments on $1{,}980$ frames of OPV2V under AWGN and Rayleigh channels show three central findings. First, fixed feature-level transmission with separate LDPC + QAM coding is strictly dominated by object-level communication at all SNRs in the evaluated range, because the LDPC cliff suppresses realised F1 below the ego-only floor. Second, the channel-state features $\hat\gamma_t$ and $c_t$ jointly account for $65\%$ of the selector's Gini importance, with $\hat\gamma_t$ alone at $40.5\%$, demonstrating that the granularity decision is fundamentally communication-side rather than scene-side. Third, the $256$-QAM action is dominated under LDPC 1/2 across the evaluated regime, motivating modulation and code-rate co-design as future work.

Beyond the specific results, the paper highlights a design principle for vehicular cooperative perception: the most informative message is not always the most useful one. Choosing the right semantic granularity for the current channel state can recover near-oracle detection performance with one to two orders of magnitude lower bandwidth than fixed feature-level transmission.

...
uniform}: it is ahead of a threshold tuned on validate for the same target budget at $B_{\max}=0.10$ and behind it at $0.20$
and $0.30$, where the threshold spends $1.53\times$ and $1.47\times$ the channel use. What is
...
$1.36\times$ the channel use (Section~\ref{sec:ablation}). We attribute the small accuracy margin
...
the same thing as channel-side sufficiency. We therefore frame \method{} as a

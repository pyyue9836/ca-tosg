...
state defaults to the robust $L$ message. \textbf{Prior-engine table:} these rows come from the
retired v3 ablation harness (v3 selector, v3 per-frame tables) and were not re-derived under the
single-collaborator corrigendum of Section~\ref{sec:exp}; they are reported as \emph{directional}
robustness evidence and their absolute magnitudes are not on the same accounting basis as the
headline tables. The committed prior-engine CSVs do not reproduce these exact magnitudes either
(they give $-0.0021$, $-0.0148$ and $-0.0568$ for the three rows), so the table is retained for its
\emph{direction and ordering} only and is flagged as an open item in
\texttt{docs/p0\_corrigendum.md}.}

...
Fig.~\ref{fig:decision_ratio} shows the per-mode selection ratios $\rho_s$ against SNR, with the frozen selector overlaid on that budget's payload-penalised oracle and the stacked-area panel showing the share of each action of $\mathcal{S}=\{E,L,F\}$ at every SNR. Under AWGN the selector's adaptation knee is at $10$~dB, where the frame-error cliff clears: $\rho_F$ steps from $0$ to $0.472$ on validate ($0.433$ on test, $0.160$ on Culver-City) and is flat above it, with $\rho_L$ falling correspondingly. Under Rayleigh $\rho_L$ stays at essentially $1$ across the whole range. The panel also makes the $E$-collapse of Section~\ref{sec:pareto} visible: under Rayleigh the oracle spends a substantial share on the ego-only action---$\rho_E=0.157$ on test and $0.133$ on Culver-City---while the frozen selector's $\rho_E$ is $0.002$ and $0.000$. $E$ is a deployed action that the imitation objective almost never selects, which is a limitation of the trained policy rather than of the action set.

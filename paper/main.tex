...
Fig.~\ref{fig:feat_imp} and Table~\ref{tab:feat_imp} report the top Gini feature importances of the deployed selector. The two channel-side features are the two \emph{individually strongest} cues: the channel-type indicator $c_t$ contributes $24.8\%$ of importance and the estimated SNR $\hat\gamma_t$ a further $22.3\%$. The next strongest cue, the scene-side \texttt{pcd\_mean\_range}, contributes only $3.9\%$, and no individual perception-side cue exceeds $4\%$. In aggregate, however, the $21$ perception-side cues carry the larger share---$52.9\%$ against $47.1\%$ for the two channel-side features---so the accurate reading is one of concentration rather than dominance: the channel side puts its mass in two decisive splits while the task side spreads comparable total mass across many weak ones. That the channel-type indicator $c_t$ outranks the estimated SNR $\hat\gamma_t$ is consistent with the frame-level channel physics: under Rayleigh fading the feasible set collapses to $L$ regardless of SNR, so the channel-type flag is the single highest-information split for the decision.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig_feature_importance.pdf}
    \caption{Top-12 features by Gini importance of the frozen $B_{\max}=0.20$ selector (\texttt{data/p2/selector\_B020.pkl}, the pre-registered confirmatory operating point). The channel-side features (red) are the two individually strongest cues and jointly account for $47.1\%$ of total importance; the $21$ perception-side LiDAR cues (blue) carry the remaining $52.9\%$ between them, none exceeding $3.9\%$ on its own.}
    \label{fig:feat_imp}
\end{figure}

\begin{table}[t]
\centering
\caption{Top-7 features by Gini importance for the deployed \method{} selector (trained on OPV2V validate, $T=1{,}980$ frames). The two channel-side features are the individually strongest cues and account for $47.1\%$ of total importance; the $21$ perception-side cues carry the remaining $52.9\%$. Read directly from the frozen model, \texttt{results/main/feature\_importance\_frozen.csv}.}
\label{tab:feat_imp}
\begin{tabular}{lc}
\toprule
Feature & Gini importance \\
\midrule
\texttt{channel\_is\_rayleigh}    & \textbf{0.248} \\
\texttt{est\_snr\_db}             & \textbf{0.223} \\
\texttt{pcd\_mean\_range}         & 0.039 \\
\texttt{pcd\_std\_range}          & 0.039 \\
\texttt{ego\_num\_objects}        & 0.037 \\
\texttt{pcd\_front\_far\_30m}     & 0.033 \\
\texttt{pcd\_back\_points}        & 0.032 \\
\midrule
...
Random Forest runs in $59.9\pm5.3$~ms per frame ($\mathrm{P95}=66.6$~ms) on a single CPU

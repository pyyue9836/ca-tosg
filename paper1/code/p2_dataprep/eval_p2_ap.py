...
This is the AP companion to eval_p2_deploy.py. It is DESCRIPTIVE: the R9 decision was made once
(eval_p2_deploy.py, r9_decision.csv) and is NOT revisited here. No confirmatory / adjudicative
...
Run:  /path/to/env/python paper1/code/p2_dataprep/eval_p2_ap.py
"""
import hashlib
import os
import sys
...
import eval_p2_deploy as D  # bit-identical selector + CSI machinery (single source)
...
def main():
    ...
    with open(os.path.join(OUT, 'PROVENANCE_ap.txt'), 'w') as f:
        f.write('CA-TOSG P2 submit-B / 6d -- TRUE end-to-end AP (eval_p2_ap.py). DESCRIPTIVE ONLY.\n' + '=' * 78 + '\n')
        f.write(f'manifest: results/p2_dataprep/FROZEN_MANIFEST.json (schema {man["schema"]}, freeze {man["freeze_timestamp"]})\n')
        f.write('Selector application + CSI replay are BIT-IDENTICAL to eval_p2_deploy.py '
    ...
    print('\nwrote results/p2_deploy/true_e2e_ap.csv + PROVENANCE_ap.txt', flush=True)

...
eval_p2_deploy, so it reproduces the mainline replay_summary EXACTLY (sanity, p3_baseline_sanity.csv).
...
mean +/- std over N_REPLAY realisations):
  ...
  5. Rician (if table)  : K in {0,3,10} from results/bler_sionna/bler_sionna_rician.csv; selector fed
                          channel_is_rayleigh=1 (fading); eff_F recomputed under the Rician frame-BLER.

Outputs (results/p3_sensitivity/): item{1..5}_*.csv + p3_baseline_sanity.csv + PROVENANCE_p3.txt.
Run:  /path/to/env/python paper1/code/p2_dataprep/eval_p3_sensitivity.py
"""
import os
import sys
...
def main():
    ...
    pd.DataFrame(sanity).to_csv(os.path.join(OUT, 'p3_baseline_sanity.csv'), index=False)
    pd.DataFrame(rows1).to_csv(os.path.join(OUT, 'item1_channel_ratio.csv'), index=False)
    pd.DataFrame(rows2).to_csv(os.path.join(OUT, 'item2_nonuniform_snr.csv'), index=False)
    pd.DataFrame(rows3).to_csv(os.path.join(OUT, 'item3_misclass_flip.csv'), index=False)
    pd.DataFrame(rows4).to_csv(os.path.join(OUT, 'item4_bler_L.csv'), index=False)
    ...
    if rt_ready:
        ...
        pd.DataFrame(rows5).to_csv(os.path.join(OUT, 'item5_rician.csv'), index=False)
        rician_done = True

    with open(os.path.join(OUT, 'PROVENANCE_p3.txt'), 'w') as f:
        f.write('CA-TOSG P3 -- descriptive sensitivity batch (eval_p3_sensitivity.py). DESCRIPTIVE ONLY.\n' + '=' * 80 + '\n')
        f.write(f'manifest: results/p2_dataprep/FROZEN_MANIFEST.json ({man["schema"]}, freeze {man["freeze_timestamp"]})\n')
        ...
        f.write(f'Baseline (uniform SNR, 50/50, BLER_L=0) uses rng(CSI_SEED={CSI_SEED}) in eval_p2_deploy order\n'
                '-> reproduces replay_summary.csv EXACTLY (p3_baseline_sanity.csv).\n')
        ...
        f.write(f'item5 Rician: {"present -> item5_rician.csv" if rician_done else "table ABSENT -> SKIPPED"} '

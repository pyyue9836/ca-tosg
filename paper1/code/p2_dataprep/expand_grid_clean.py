...
Per split (validate/test/culver):
  ...
  3. Expand each frame across the DETERMINISTIC grid  SNR {0,2,..,20} dB x channel {AWGN,Rayleigh}
     (11 x 2 = 22 cells/frame). For each cell compute, under S = {E, L, F}:
        ...
        oracle  = argmax over [E, L, F]  (E is a first-class action; ties -> the earlier/cheaper
                  action, so a fully-failed F (eff_F==ego) yields E, never a paid-for dead request)
  4. Write data/p2/p2_grid_{split}.csv (git-excluded) + results/p2_dataprep/PROVENANCE_grid.txt.
...
def expand_split(split, tbl):
    ...
    for ch in CHANNELS:
        for snr in SNR_GRID:
            ...
            stack = np.stack([eff_E, eff_L, eff_F], axis=1)      # order = [E, L, F]
            oracle = np.array(ACTIONS)[stack.argmax(1)]          # ties -> E, then L
...
def main():
    ...
    results = [expand_split(sp, tbl) for sp in SPLITS]
    for r in results:
        print(f"[{r['split']}] frames={r['n_frames']} scenes={r['n_scenes']} rows={r['rows']} "
              f"oracle base-rate(E/L/F)={r['base_rate']}  md5={r['md5']}")
    with open(os.path.join(OUT_PROV, 'PROVENANCE_grid.txt'), 'w') as f:
        f.write("CA-TOSG P2 data prep -- clean-cache grid expansion (expand_grid_clean.py)\n")
        f.write("=" * 72 + "\n")
        ...
        f.write("oracle_ELF = argmax over [E,L,F] (ties -> E then L). NOT the legacy oracle_3way {L,C16,C256}.\n")
    ...
    print('wrote', os.path.join(OUT_PROV, 'PROVENANCE_grid.txt'))

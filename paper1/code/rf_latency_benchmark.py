...
eval_p2_deploy.py) and measures the deployment predict() call.
...
End-to-end timing table (e2e_timing.csv). One row per per-frame pipeline stage, each tagged
...
Outputs (results/p2_latency/):
  latency_frozen.csv  -- per frozen selector: mean/std/P50/P95/P99 ms + size_kb (batch-1).
  e2e_timing.csv      -- per-frame pipeline timing table with per-row measured/calculated/assumed/
                         not-included tags + source (selector row filled from latency_frozen.csv).
  PROVENANCE_latency.txt

Run:  /path/to/env/python paper1/code/rf_latency_benchmark.py
"""
import io
import os
import pickle
...
import eval_p2_deploy as D          # frozen-manifest loader + feature-matrix builder (single source)

P1 = D.P1
OUT_DIR = os.path.join(P1, 'results/p2_latency')
...
def main():
    ...
    lat.to_csv(os.path.join(OUT_DIR, 'latency_frozen.csv'), index=False)
    ...
    e2e = [
        ...
        dict(stage='Selector inference (RF predict; batch-1)',
             ...
             source=f'latency_frozen.csv; slowest frozen selector (mean; P95={sel_p95:.2f} ms); '
    ...
    pd.DataFrame(e2e).to_csv(os.path.join(OUT_DIR, 'e2e_timing.csv'), index=False)

    with open(os.path.join(OUT_DIR, 'PROVENANCE_latency.txt'), 'w') as f:
        f.write('CA-TOSG P2-C -- frozen-selector latency + end-to-end timing (rf_latency_benchmark.py)\n' + '=' * 80 + '\n')
        ...
        f.write('e2e_timing.csv: per-frame pipeline, one row per stage, tag in '
    ...
    print('wrote results/p2_latency/{latency_frozen.csv,e2e_timing.csv,PROVENANCE_latency.txt}', flush=True)

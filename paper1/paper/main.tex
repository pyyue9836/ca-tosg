...
evaluation frames. Evaluated on the frozen test split, under LDPC + QAM the edge is $+0.005$ F1 ($95\%$ CI
$[+0.004,+0.006]$): small, and---as the feature ablation shows---not attributable to the
perception cues, which add no F1 over channel state alone (Section~\ref{sec:ablation}); it
is the margin of a learned selector over even an oracle-tuned single threshold.\footnote{This
per-frame edge over an oracle-tuned threshold ($+0.005$) is distinct from, and must not be
...
($[+0.020,+0.025]$) under Rayleigh, and $+0.025$ ($[+0.023,+0.028]$) under OFDM (all
significant): a gain not attainable by SNR thresholding alone.\footnote{The JSCC edge is

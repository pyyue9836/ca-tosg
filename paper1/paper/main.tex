...
\begin{figure*}[t]
    ...
    \caption{Overview of the proposed \method{} framework. The sender extracts online task cues $z_t$ and estimated channel quality $(\gamma_t,\text{ch}_t)$, then selects one communication mode $s_t \in \{L,C_{16}\}$ (the dominated $C_{256}$ mode is excluded from the deployed action set, Section~\ref{sec:candidates}). Only the selected message is transmitted through the V2V link and fused with the local ego feature $F_{\mathrm{ego},t}$ for 3D object detection.}
    \label{fig:overview}
\end{figure*}

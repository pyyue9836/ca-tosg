...
\multirow{2}{*}{Validate ($1{,}980$)}
  & Fallback ($L$)  & 0.893 & 0.839 & 0.024 \\
  & AWGN $\ge14$~dB & \textbf{0.911} & \textbf{0.865} & 0.339 \\
\midrule
\multirow{2}{*}{Test ($2{,}170$)}
  & Fallback ($L$)  & 0.905 & 0.856 & 0.024 \\
  & AWGN $\ge14$~dB & \textbf{0.920} & \textbf{0.865} & 0.340 \\
\midrule
\multirow{2}{*}{Culver-City ($550$)}
  & Fallback ($L$)  & 0.811 & 0.724 & 0.024 \\
  & AWGN $\ge14$~dB & \textbf{0.865} & \textbf{0.771} & 0.402 \\
\bottomrule
\end{tabular}
\end{table}

Three observations follow. (i) When the channel can carry features, the selector captures the cooperative AP gain: AP@0.5 rises by $+0.018$ (validate), $+0.015$ (test), and $+0.054$ (Culver-City) over the object-level baseline, and AP@0.7 by $+0.026$, $+0.009$, and $+0.047$, respectively---the gain is largest on the harder Culver-City domain. On validate and test this sits within $0.006$ and $0.002$~AP@0.5 of the perfect-channel feature-level ceiling ($0.917$ and $0.922$). (ii) Under fading or low SNR the selector holds $L$ and incurs no AP loss relative to object-level communication, so feature-level transmission is never activated where it would be wasteful; the fixed feature-level policies $C_{16}$ and $C_{256}$, by contrast, are \emph{strictly dominated}---they cost $10\text{--}21\times$ more bandwidth yet fall below Fixed $L$ in realised AP because of the LDPC cliff (Fig.~\ref{fig:ap_snr}). (iii) Averaged over all channel states the selector's payload is $\le0.085$~Mbit/frame, i.e.\ $\le17\%$ of Fixed $C_{16}$, because the expensive feature-active regime occupies only the high-SNR AWGN tail.

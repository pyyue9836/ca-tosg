...
evaluation frames. Evaluated on the frozen test split, under LDPC + QAM the AWGN edge is $+0.005$ F1 ($95\%$ CI
$[+0.004,+0.006]$): small, and---as the feature ablation shows---not attributable to the
perception cues, which add no significant F1 over channel state alone (Section~\ref{sec:ablation}); it
is the margin of a learned selector over even an oracle-tuned single threshold.\footnote{This
per-frame edge over an oracle-tuned threshold ($+0.005$) is distinct from, and should not be
conflated with, two other LDPC-regime quantities: the feature-ablation gain of the cues
(Full vs.\ channel-state-only, $-0.0002$, Section~\ref{sec:ablation}) and the matched-payload
Pareto margin over the retuned threshold ($+0.0005$, Section~\ref{sec:headline}). The three
measure the selector along the cue, the equal-bandwidth, and the per-frame axis respectively;
the ordering $-0.0002 < +0.0005 < +0.005$ is itself informative.} Under JSCC the best SNR
threshold collapses to the Fixed-$L$ operating point---it cannot gate on an uninformative
SNR---and the edge widens to $+0.027$ F1 ($[+0.024,+0.029]$) under AWGN, $+0.022$
($[+0.020,+0.025]$) under Rayleigh, and $+0.025$ ($[+0.023,+0.028]$) under OFDM (all CIs exclude zero): a gain not attainable by SNR thresholding alone.\footnote{The JSCC edge is
split-dependent (test $+0.027$, validate $+0.004$ under AWGN): on validate the Fixed-$L$
baseline already sits close to the selector's JSCC F1 ($0.907$ vs.\ $0.911$), leaving little
to recover, whereas on test the selector reaches $0.928$ over a $0.901$ baseline.}
...
\begin{figure}[t]
    ...
    (\method), and the oracle, on the same $2{,}170$ test frames under each feature
    codec. Under LDPC the threshold closely tracks \method; under JSCC the threshold collapses to

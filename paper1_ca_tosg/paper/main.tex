...
\method{} is not designed to maximise average F1 beyond Fixed $L$ on every frame: on many frames object-level communication is already sufficient. Its value is to select feature-level communication \emph{only} when the expected task gain justifies the payload and channel risk. The appropriate figure of merit is therefore the payload--F1 frontier (Section~\ref{sec:pareto}) rather than raw mean F1 alone, and the per-frame gain is concentrated where it matters (Section~\ref{sec:difficulty}).
...
factor}, and a simple, fully interpretable SNR threshold already captures it. The value the
ego-side perception cues add is \emph{frame-selective}, not aggregate: on hard frames under a
usable channel (Section~\ref{sec:difficulty}) the RF's gain over Fixed~$L$ is
$+0.045$ F1 with bootstrap $95\%$ CI $[+0.033,+0.059]$ (significant), because it sends
features to the frames that benefit rather than to every high-SNR frame. We therefore frame
\method{} as a \emph{lightweight, interpretable channel-aware selection policy} whose
aggregate operating point matches a threshold and whose learned refinement helps
significantly on the hard-frame subset. Dropping the range, density, or object-count cue
groups changes F1 by $<0.001$. (The single-realisation payloads in
Table~\ref{tab:ablation_threshold} are one such draw; the channel-averaged RF payload is
$0.078$, CI $[0.072,0.084]$.)

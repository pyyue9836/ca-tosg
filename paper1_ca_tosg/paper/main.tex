...
\begin{itemize}
    ...
    \item Our central finding is that whether task-oriented perception cues are needed for granularity selection is \emph{determined by the feature codec's channel response}. Under an LDPC + QAM cliff the estimated SNR is a near-sufficient statistic and a one-line threshold matches the learned selector (the cues add $<0.001$ aggregate F1); under graceful importance-map JSCC the SNR becomes uninformative, the best SNR threshold collapses to object-level performance, and the cue-based selector beats it by $+0.017$ F1 ($95\%$ CI $[+0.012,+0.022]$, AWGN; $+0.015$ under Rayleigh). We report this honestly alongside supporting selector-design analyses (feature ablations, model/latency comparisons, Rician/CSI-aging robustness) and a negative finding that 256-QAM is dominated under LDPC 1/2.
\end{itemize}
...
We consider a single ego--collaborator V2V link. At each frame $t$, the ego vehicle observes the scene using its onboard sensors and extracts a local bird's-eye-view (BEV) feature representation, denoted by $F_{\mathrm{ego},t}$. The collaborating vehicle also observes the scene from its own viewpoint and extracts a local feature representation $F_{j,t}$ using a shared PointPillars backbone~\cite{lang2019pointpillars}. The collaborator can transmit a perception message to the ego vehicle through the V2V link. Multi-vehicle shared-bandwidth scheduling is a system-level extension discussed in Section~\ref{sec:discussion}. The ego vehicle then fuses the received message with its local feature and produces the final 3D object detection output.
...
\begin{figure}[t]
    ...
    \caption{Block-error rate of the feature-level message under LDPC $1/2$ with 16-QAM and 256-QAM, over AWGN (solid) and Rayleigh (dashed) channels. The 16-QAM AWGN cliff at $\approx 12$~dB defines the selector knee (shaded); the 256-QAM cliff sits far to its right, so by the time 256-QAM is reliable the 16-QAM block is already error-free---this is the basis of the 256-QAM dominance discussed in Appendix~\ref{sec:negative}. Under Rayleigh fading both curves degrade so slowly that feature-level transmission never becomes reliable in $0$--$20$~dB, explaining the selector's conservative $L$-only behaviour.}
    \label{fig:bler}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Headline results: true end-to-end AP of \method{} across three OPV2V splits, in the fading/low-SNR fallback regime (selector holds $L$) and the AWGN $\hat\gamma_t\ge14$~dB feature-active regime (selector requests $C_{16}$). When the channel can carry features, \method{} lifts AP toward the feature-level ceiling; when it cannot, it safely retains the $L$ operating point. Payload is channel-use-equivalent Mbit/frame; channel-\emph{averaged} \method{} payload is $\le0.085$~Mbit/frame, i.e.\ $\le17\%$ of Fixed $C_{16}$ ($0.495$).}
\label{tab:headline}
\begin{tabular}{llccc}
\toprule
...
\multirow{2}{*}{Validate ($1{,}980$)}
  & Fallback ($L$)  & 0.838 & 0.757 & 0.024 \\
  & AWGN $\ge14$~dB & \textbf{0.863} & \textbf{0.791} & 0.339 \\
\midrule
\multirow{2}{*}{Test ($2{,}170$)}
  & Fallback ($L$)  & 0.859 & 0.781 & 0.024 \\
  & AWGN $\ge14$~dB & \textbf{0.881} & \textbf{0.795} & 0.340 \\
\midrule
\multirow{2}{*}{Culver-City ($550$)}
  & Fallback ($L$)  & 0.799 & 0.668 & 0.024 \\
  & AWGN $\ge14$~dB & \textbf{0.852} & \textbf{0.729} & 0.402 \\
\bottomrule
\end{tabular}
\end{table}

Three observations follow. (i) When the channel can carry features, the selector captures the cooperative AP gain: AP@0.5 rises by $+0.025$ (validate), $+0.022$ (test), and $+0.053$ (Culver-City) over the object-level baseline, and AP@0.7 by $+0.034$, $+0.014$, and $+0.061$, respectively---the gain is largest on the harder Culver-City domain. On validate and test this sits within $0.007$ and $0.018$~AP@0.5 of the perfect-channel feature-level ceiling ($0.870$ and $0.899$). (ii) Under fading or low SNR the selector holds $L$ and incurs no AP loss relative to object-level communication, so feature-level transmission is never activated where it would be wasteful; the fixed feature-level policies $C_{16}$ and $C_{256}$, by contrast, are \emph{strictly dominated}---they cost $10\text{--}21\times$ more bandwidth yet fall below Fixed $L$ in realised AP because of the LDPC cliff (Fig.~\ref{fig:ap_snr}). (iii) Averaged over all channel states the selector's payload is $\le0.085$~Mbit/frame, i.e.\ $\le17\%$ of Fixed $C_{16}$, because the expensive feature-active regime occupies only the high-SNR AWGN tail.
...
Because a one-line SNR-threshold rule is the natural challenger to any learned selector, we place it in the headline rather than an appendix. Table~\ref{tab:headline_agg} reports the channel-averaged frame F1 and payload of Fixed $L$, an SNR-threshold rule, the learned selector, and the oracle over $200$ random SNR/channel draws on the frozen test split. The honest reading is that, \emph{averaged over all channel states, the learned selector and the threshold rule lie on the same payload--F1 frontier}---at equal F1 the threshold is in fact marginally cheaper ($0.889$ at payload $0.071$ versus the RF's $0.078$). On an LDPC + QAM channel the estimated SNR is therefore a near-sufficient statistic for the granularity decision, and the $21$ ego-side perception cues add no aggregate bandwidth advantage over channel state alone. We report this rather than hide it: it is the central diagnostic finding of the paper. The learned selector's residual value is (i) frame-selective---the granularity policy's gain over Fixed~$L$ reaches $+0.045$ F1 (bootstrap $95\%$ CI $[+0.033,+0.059]$) on hard frames under a usable channel (Section~\ref{sec:difficulty})---and (ii) decisive on channels without a sharp SNR cliff, such as JSCC: when SNR ceases to be a sufficient statistic the cue-based selector beats the best SNR threshold by $+0.017$ F1 (Section~\ref{sec:jscc_aware}).

\begin{table}[t]
\centering
\caption{Channel-averaged comparison on the frozen OPV2V \emph{test} split (selector trained on validate, \emph{not} retrained; $T=2{,}170$ frames). Mean realised frame F1 and channel-use-equivalent payload over $200$ random draws of per-frame SNR ($\mathcal{U}[0,20]$~dB) and channel type ($50/50$ AWGN/Rayleigh); payload $95\%$ CI in brackets, F1 CI half-widths $<0.0011$ for the learned policies. The SNR-threshold rule lies on the same payload--F1 frontier as the learned selector.}
\label{tab:headline_agg}
\begin{tabular}{lcc}
\toprule
Policy & Mean F1 & Payload (Mbit/frame) \\
\midrule
Fixed $L$                    & 0.887 & 0.024 \\
SNR-threshold ($\tau{=}16$)  & 0.889 & 0.071\,[0.066,\,0.077] \\
\method{} (RF, frozen)       & 0.889 & 0.078\,[0.072,\,0.084] \\
Channel-aware oracle         & \textbf{0.894} & 0.074 \\
\bottomrule
\end{tabular}
\end{table}
...
(train on validate, evaluate on test). We report this honestly. \emph{Channel-averaged,
the SNR-threshold rule matches the learned RF on the payload--F1 frontier}: over $200$
random SNR/channel draws the RF attains mean F1 $0.8886$ ($95\%$ CI $[0.8878,0.8895]$) at
mean payload $0.078$, and a tuned threshold reaches the same F1 at essentially the same
payload---the learned selector gives \emph{no channel-averaged Pareto advantage} over the
threshold. This is the key honest message: \emph{channel awareness is the dominant decision
factor}, and a simple, fully interpretable SNR threshold already captures it. The $21$
ego-side perception cues add $<0.001$ aggregate F1 over channel state alone on this
LDPC + QAM channel; we attribute this to the sharp SNR cliff, which makes the estimated
SNR a near-sufficient statistic for the decision. What the granularity \emph{policy}
adds over object-level communication is nonetheless frame-selective, not aggregate: on
hard frames under a usable channel (Section~\ref{sec:difficulty}) the policy's gain over
Fixed~$L$ is $+0.045$ F1 with bootstrap $95\%$ CI $[+0.033,+0.059]$ (significant). We
therefore frame \method{} as a \emph{lightweight, interpretable channel-aware selection
policy} whose aggregate operating point matches a threshold on this cliff codec; the
...
$+0.017$ F1 (Section~\ref{sec:jscc_aware}). Dropping the range, density, or object-count cue
groups changes F1 by $<0.001$; the channel-averaged payloads, with $95\%$ CIs over
$200$ seeds, are in Table~\ref{tab:headline_agg} (RF $0.078$ versus the $\tau{=}16$
threshold's $0.071$).
...
($1{,}980$ frames) using our $50$-epoch reproduction. Under perfect
channel (lossless feature-level transmission), Where2comm achieves
AP@0.5 = $0.885$ and AP@0.7 = $0.811$, the strongest perfect-channel
feature-level result in our study---above the ImportanceMapJSCC
reference of $0.866 / 0.728$, the attentive-compression branch used by
\method{} ($0.870$ AP@0.5), and the channel-invariant Fixed $L$ value of
$0.840 / 0.760$. This is expected: Where2comm is a well-engineered
intermediate-fusion method, and its strength under a clean channel
confirms that our evaluation pipeline does not handicap feature-level
baselines. The key point is that this strength is \emph{orthogonal} to
\method: Where2comm is a spatial-mask method that decides \emph{which
feature regions} to send \emph{within} a fixed feature-level granularity,
whereas \method{} decides \emph{whether} to send feature-level
information at all and at \emph{what} granularity, given the channel.
The two compose: Where2comm can be deployed inside the $C_q$ branches of
\method{} as a stronger feature codec, while the granularity selector
decides when feature-level communication is worth its payload and channel
risk. Under a bandwidth-constrained, time-varying link, even a
perfect-channel AP of $0.885$ is realised only when the channel can
deliver the feature message---precisely the regime \method{} arbitrates.
...
($C_{256}^{3/4}$, Appendix~\ref{sec:negative}); (iv) a Where2comm-style spatial
...
\section{Robustness to CSI Imperfection and Request Delay}\label{sec:csi_noise}

Table~\ref{tab:robustness} consolidates the selector's sensitivity to the three
practical imperfections of a deployed V2X link---a noisy SNR estimate, channel-state
aging under mobility, and a stale (delayed) request---each detailed below and in
Appendices~\ref{sec:vehicular} and~\ref{sec:causality}. In all three cases the realised
F1 degrades gracefully and the loss is bounded, because whenever the channel state is
uncertain the selector falls back to the robust object-level message $L$.

\begin{table}[t]
\centering
\caption{Robustness summary (OPV2V validate; realised frame F1). Each row perturbs one
deployment imperfection while holding the others ideal. Losses are bounded because an
uncertain channel state defaults to the robust $L$ message.}
...
\multirow{3}{*}{SNR-estimation noise $\sigma$}
   ...
   & $5$~dB                  & $-0.070$ \\
\midrule
CSI aging (Jakes, $60$~km/h) & stale by ${\sim}10$~ms & $-0.015$ (then plateaus) \\
\midrule
Request delay & $1$-frame-stale decision & $\ge -0.057$ (pessimistic i.i.d.\ bound) \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Selector Model Comparison}\label{sec:models}

Table~\ref{tab:models} compares candidate selectors at batch size $1$ (the $10$~Hz
operating point; latencies on the host of Appendix~\ref{sec:latency}). All models reach
...
size 1 on the experimental host; realised F1 is essentially model-independent, so the
models trade only latency and size. The rigorous channel-averaged payload comparison
($200$ seeds, with CIs) is in Table~\ref{tab:headline_agg}.}\label{tab:models}
\begin{tabular}{lccc}
\toprule
Model & Realised F1 & Latency (ms) & Size \\
\midrule
Decision Tree          & 0.889 & 0.40 & 11~KB \\
Logistic Regression    & 0.889 & 0.55 & 2~KB \\
RBF-SVM                & 0.888 & 0.57 & 82~KB \\
Small MLP (32,16)      & 0.889 & 0.55 & 55~KB \\
Random Forest (400)    & 0.887 & 8.3 & 3.5~MB \\
SNR-threshold ($\tau{=}16$) & 0.889 & $\sim0$ & -- \\
\bottomrule
\end{tabular}
\end{table}

\section{Scene-Content Subsets and Object-Level Reliability}\label{sec:subsets}
...
\section{Vehicular Channels: Rician Fading and CSI Aging}\label{sec:vehicular}
...
CSI is unreliable. This complements the CSI-noise ablation of Appendix~\ref{sec:csi_noise}.
...
\section{Request Latency and Decision Causality}\label{sec:causality}
...
\section{Inference Latency}\label{sec:latency}
...
\section{Design Ablation: Action-Space Cliff Matching}\label{sec:negative}

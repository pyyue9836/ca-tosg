...
\midrule
Fixed $L$ (object-level)      & 0.887 & 0.024 \\
Fixed $C_{16}$ (LDPC+16-QAM)  & 0.426 & 0.495 \\
Fixed $C_{256}$ (LDPC+256-QAM)& 0.116 & 0.248 \\
...
\begin{figure*}[t]
    ...
    \caption{Detection performance versus SNR under AWGN and Rayleigh channels. \method{} (black star) approaches the channel-aware oracle (magenta diamond) under AWGN by activating $C_{16}$ when the channel can support it. Under Rayleigh fading, the selector converges to a conservative $L$-dominant policy, matching Fixed $L$ closely. LDPC + QAM separate-coding baselines fall well below Fixed $L$ across all SNRs, demonstrating that traditional digital feature-level transmission is strictly dominated by object-level communication in this regime.}
    \label{fig:ap_snr}
\end{figure*}
...
noise up to $\sigma\approx1$~dB with $\le0.003$ F1 loss; under a Jakes model at $60$~km/h the
SNR estimate decorrelates within ${\sim}10$~ms, costing $-0.015$ F1 before plateauing; and
acting on a one-frame-stale decision costs at most $-0.057$ F1 under our i.i.d.-per-frame
channel assumption. The framework also spans the full fading-severity range: replacing the
...
Section~\ref{sec:jscc_aware} reproduces under OFDM ($+0.015$ F1 edge). Finally, the deployed
...
\begin{table}[t]
\centering
\caption{Deployment robustness (OPV2V validate; realised frame F1). Each row perturbs one

...
Run:  /path/to/env/python projects/ca_tosg/datasets/grid_builder.py
"""
import argparse
import hashlib
import os
...
# --- end bootstrap ---
from scene_split import scene_labels  # noqa: E402
...
PAYLOAD_MSYM = {'E': 0.0, 'L': 0.024, 'F': 0.99}                 # report-only (see docs/experiment_protocol.md / test_payload.py)
ACTIONS = ['E', 'L', 'F']
CLEAN_COLS = {'E': 'ego_f1', 'L': 'late_f1', 'F': 'compressed_f1'}


def bler_frame(snr_arr, tbl, qam, channel):
    """Frame-level BLER at Es/N0=snr for a QAM order over a channel; mirrors opv2v.py exactly
...
def main():
    ...
    with open(prov, 'w') as f:
        f.write(f"CA-TOSG P2 data prep -- clean-cache grid expansion (grid_builder.py --split {split})\n")
        f.write("=" * 72 + "\n")

...
expectation, writes results/provenance/anomaly_report.txt, and -- per the binding handling rules --
...
Run:  /path/to/env/python projects/ca_tosg/evaluation/anomaly_check.py
"""
import json
import os
import sys
...
DEPLOY = os.path.join(P1, 'results/main')
MANIFEST = os.path.join(P1, 'results/manifests/FROZEN_MANIFEST.json')
REPORT = os.path.join(P1, 'results/provenance/anomaly_report.txt')


def main():
    man = json.load(open(MANIFEST))
    act = pd.read_csv(os.path.join(DEPLOY, 'action_distribution.csv'))
        ...
        if unmet:
            ...
            if ray_collapse:
                f.write('\nROOT CAUSE (E-collapse, diagnosed -- not an artefact hand-wave):\n'
                        ...
                        'post-unblinding diagnostic (decision_log.py) quantifies the cost against the '
    ...
    if unmet:
        raise SystemExit(f'P2-B FUSE (PROTOCOL sec 8): {len(unmet)} expectation(s) unmet '
                         '(see results/provenance/anomaly_report.txt). Halted per handling rule 1.')

...
Outputs (results/p2_deploy/, all raw + PROVENANCE):
  ...
  b. action_distribution.csv  -- per split x budget x SNR x channel: RF & oracle action fractions,
  ...
  d. (true end-to-end AP -- separate step; see end_to_end_ap.py.)
...
Run:  /path/to/env/python projects/ca_tosg/evaluation/deployment.py
"""
import hashlib
import json
import os
...
def main():
    ...
    pd.DataFrame(action_rows).to_csv(os.path.join(OUT, 'action_distribution.csv'), index=False)
    ...
    # ---- PROVENANCE ----
    with open(os.path.join(PROV_DIR, 'PROVENANCE_deploy.txt'), 'w') as f:
        f.write('CA-TOSG P2 submit-B -- deployment eval (deployment.py)\n' + '=' * 72 + '\n')

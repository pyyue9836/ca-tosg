...
fig_pareto_test.pdf  <- results/main/pareto_points.csv (test rows, corrected rate-1/2 payloads)
    ...
    selector md5 pending P2 re-freeze (see results/provenance/PROVENANCE_policy.txt; the old eb9358e9 was v2).

fig_payload_awgn.pdf <- results/main/true_e2e_global_validate.csv (AWGN CA-TOSG rows; matches the validate AP/payload section)
...
def pareto_test():
    df = pd.read_csv(os.path.join(RES, 'policy', 'pareto_points.csv'))  # corrected rate-1/2 payloads
    d = df[df.split == 'test']
    style = {
...
def payload_awgn():
    df = pd.read_csv(os.path.join(RES, 'true_e2e_global_validate.csv'))  # validate: matches item-7 payload section (0.633/0.990)

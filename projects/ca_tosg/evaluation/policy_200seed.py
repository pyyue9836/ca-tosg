...
V3 protocol (vs the v2 recompute this file used to hold):
  ...
  * BLER               : Sionna FRAME-level table results/channel/bler_sionna.csv (bler_frame
  ...
  * feasibility mask   : BLER_a >= 0.999 removes action a from the ORACLE's feasible set (label
                         ...
                         constraint used in opv2v.py. lambda stays pinned = 0.
...
def main():
    raise SystemExit(
        "policy_200seed.py is FUSED OFF (superseded by docs/experiment_protocol.md, P2 submit-A). The P2 "
        ...
        "generator-of-record for results/main/threshold_vs_rf.csv, which still backs main.tex "
    ...
    with open(os.path.join(PROV_DIR, 'PROVENANCE.txt'), 'w') as f:
        ...
        f.write("BLER: results/channel/bler_sionna.csv (frame-level bler_frame, Es/N0); "

...
results/main/r10c_missed_e_cost.csv (+ r10c_vs_oracle_account.csv) -- NO hand-written numbers.
...
the R10-report review.) Does NOT touch the CSVs, decision_log.py, or any assertion; it regenerates
results/provenance/R10_REPORT.md and PROVENANCE_r10c.txt. Run after decision_log.py.

Run:  /path/to/env/python projects/ca_tosg/evaluation/report.py
"""
import json
import os
...
def main():
    ...
    lines = ['# R10-family diagnostic report (auto-generated from the CSVs; do not hand-edit)\n',
             '_Every number is recomputed by `projects/ca_tosg/evaluation/report.py` from '
    ...
    with open(os.path.join(PROV_DIR, 'PROVENANCE_r10c.txt'), 'w') as f:
        ...
        f.write('All numbers recomputed by report.py from r10c_missed_e_cost.csv (3 class rows; '
                ...
                'budget-constrained). decision_log.py 4 hard assertions PASSED; F1_RF/B_RF reproduce the '

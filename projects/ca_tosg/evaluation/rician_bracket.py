...
the TRUE Rician frame-BLER (results/channel/bler_sionna_rician.csv, K in {0,3,10}). This is the
...
Outputs (results/p3_sensitivity/):
  item5c_rician_rayleigh0.csv  -- 200-replay aggregate (same seed), per split x budget x K: F1/std,
                                  payload, rho_feature (directly comparable to rician_proxy.csv).
...
Run:  /path/to/env/python projects/ca_tosg/evaluation/rician_bracket.py
"""
import os
import sys
...
def main():
    if not os.path.exists(RICIAN_TBL):
        raise SystemExit('P3-C FUSE: rician table absent -- run ldpc_qam.py --rician_K 0,3,10 first.')
    ...
    with open(os.path.join(PROV_DIR, 'PROVENANCE_p3.txt'), 'a') as f:
        f.write('\n[P3-C, 2026-08-11] Rician BRACKETING variant (rician_bracket.py). '
                f'"{LABEL}".\n')

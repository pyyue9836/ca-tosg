...
Candidate set PARSED from docs/experiment_protocol.md's ```json CATOSG-CANDIDATES``` block (single source of truth).
...
CPU only, deterministic (seed from the JSON block). Run:
  /path/to/env/python projects/ca_tosg/models/selector.py
"""
import hashlib
import itertools
import json
...
from projects.ca_tosg.models.feature_encoder import CUES_CSV, GRID_CSV, load
from projects.ca_tosg.models.oracle import ACTIONS, PAYVEC, lam_labels
...
OUT_PROV = os.path.join(P1, 'results/manifests')
...
def parse_candidates():
    ...
    if not m:
        raise SystemExit('CATOSG-CANDIDATES block not found in docs/experiment_protocol.md.')
...
def main():
    ...
    if violations:                                                   # walk exhausted for some budget
        ...
        with open(os.path.join(PROV_DIR, 'PROVENANCE_train.txt'), 'w') as f:
    ...
    walk_files = {f'B{int(round(b*100)):03d}': os.path.join(OUT_PROV, f'candidate_walk_B{int(round(b*100)):03d}.csv')
                  for b in spec['budgets']}
    manifest = dict(
        schema=MANIFEST_SCHEMA, protocol='CA-TOSG P2 (docs/experiment_protocol.md), R9',
        ...
        inputs=dict(
            ...
            bler_table=dict(file='results/channel/bler_sionna.csv', md5=_md5(BLER_CSV)),
    ...
    with open(os.path.join(PROV_DIR, 'PROVENANCE_train.txt'), 'w') as f:
        f.write('CA-TOSG P2 submit-A (R9) -- frozen-walk selection + freeze, atomic (selector.py)\n')
        f.write('=' * 72 + '\n')

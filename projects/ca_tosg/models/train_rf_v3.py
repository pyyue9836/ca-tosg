...
ego fallback + lam=0 oracle labels from opv2v.py). Held-out 30% (stratified, SEED=0) gives
...
loaders (policy_200seed.py, true_e2e_global.py) that do pickle.load(...).predict(...).
...
Also retains the base/csi/full ablation comparison in results/main/step4_rf_modes.csv.
"""
import hashlib
import json
import os
...
def main():
    raise SystemExit(
        "train_rf_v3.py is FUSED OFF (superseded by docs/experiment_protocol.md, P2 submit-A). Use "
        "projects/ca_tosg/models/selector.py (LOSO 9-fold + budget-matched tau* + one model per "
    ...
    print('acceptance -> results/main/step4_rf_class_report.csv, step4_rf_modes.csv, '

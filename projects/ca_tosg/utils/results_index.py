...
RULES = [
    ...
    (r'^v2/(wp5_final|wp5_message|wp5_f_products)_\w+\.(csv|json)$',
     'python projects/ca_tosg/evaluation/v2_wp5_final.py',
     'work package 5: F products, three delivery regimes, R=4'),
    (r'^v2/wp5_tpfp_\w+\.npz$', 'python projects/ca_tosg/evaluation/v2_wp5_message.py',
     'per-frame tp/fp/score decompositions -- the one forward cost, reused not recomputed'),
    (r'^v2/(diverged_frames|seed_collision_scan)\.json$',
     'python tests/test_eval_determinism.py',
     'determinism gate inputs: frames that diverged pre-fix, and the seed collision scan'),
    (r'^v2/diagnostic/.*$', 'superseded -- see results/v2/diagnostic/README.md',
     'DIAGNOSTIC ONLY: generated before the shuffle_points fix, not reproducible'),
    (r'^manifests/V2_SEALED_HASHES\.json$', 'python tests/test_sealed_heldout.py --write-hashes',
     'sha256 of each sealed held-out file, compared at unseal time (V2-R6 A-2)'),
    (r'^manifests/V2_RUNTIME_MANIFEST\.json$', 'python tools/build_v2_protocol_manifest.py',
     'runtime switches the products depend on (V2-R17 C)'),
    ...
    (r'^manifests/(V2_FROZEN_MANIFEST\.json|v2_validate_loso_folds\.csv)$',
     'python projects/ca_tosg/models/v2_selector.py',
     'V2-R26: v2 selector LOSO folds + per-budget walk on v2_ego_local_23d (validate only)'),
    (r'^manifests/V2_CUE_SCHEMA\.json$',
     'python tools/build_cue_schema_manifest.py',
     'V2-R24 B-2: the frozen v2_ego_local_23d schema -- fields, order, FOV, pinned dependencies'),

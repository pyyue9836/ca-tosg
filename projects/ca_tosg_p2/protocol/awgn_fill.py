...
def render(d):
    ...
    w('')
    w('* `p_cw_point` is the **empirical point estimate** of the codeword error rate, k/N. It is the')
    w('  column the main analysis uses to construct the message success probability and the expected')
    w('  perception outcome. Where no error was observed it is exactly `0.0`, and that zero means')
    w('  "none seen in N draws", never "the rate is zero". **k/N is unbiased for p itself, but')
    w('  `q = (1 - p)^12567` is a non-linear function of it, so `(1 - k/N)^12567` is not an unbiased')
    w('  estimate of q** — with a convex transform of this steepness the plug-in value sits above the')
    w('  mean of the sampling distribution. It is a point estimate and is reported as one.')
    ...
    w('')
    w('**An error-rate bound is not a non-inferiority result.** Knowing that p_cw lies below some value')
    w('says nothing on its own about whether one action beats another: that requires the difference in')
    w('expected effect between two policies, computed at the same channel parameters, with the')
    w('between-scene variation carried through. That comparison is in `awgn_fill_eval.md`, not here.')
    w('')

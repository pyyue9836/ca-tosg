...
Durable record of the R9 decision result and its **verbatim allowed wording**. `docs/claims.md` is
auto-generated from `main.tex`, so this file (not a hand-added `docs/claims.md` row) is the authority for
the wording; when the sentence lands in `main.tex` at P5 it must use the locked text below and its
`docs/claims.md` row's *Allowed wording* column points here. Source data:
`results/main/replay_summary.csv` + `replay_test_B020.csv` (paired bootstrap, 10,000 resamples,

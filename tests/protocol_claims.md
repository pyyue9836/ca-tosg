...
| w2c-no-verdict | The confirmatory framing is withdrawn | false-as-written | `[Ww]here2comm[^.\n]{0,90}(?<!not )(?<!could not be )(adjudicat&#124;verdict&#124;confirmatory (win&#124;loss&#124;result))` | could not be performed | R57: the confirmatory cell was never run -- the comparator's threshold has no setting within +-20% of the 0.20 cap, bracket [0.0802, 0.3130] on test. Any sentence claiming a verdict for this arm contradicts the record. |
| same-transport-claim | the arm was never run under the transport | false-as-written | `(Where2comm&#124;external comparison)[^.\n]{0,90}under (this paper's&#124;the same&#124;our) (own )?transport` | ideal delivery | R58: a sentence claiming the external arm ran under this paper's transport is only true if results/diagnostics/transport_replay.csv exists and the sentence is not describing the ideal-delivery tables. The R57 text made that claim with no replay in the repository at all. |
| per-frame-accounting | per-frame `comm_rate[k]` | false-as-written | `charged at the split's mean&#124;a single \$?N_\{?\\mathrm\{cw\}\}?\$? for the whole split` | per frame | R60-1: the replay charges each frame at its own transmitted fraction. A sentence saying otherwise contradicts transport_replay.py, which reads the comm_rate ARRAY (zc['comm_rate']) and builds n_cw_pf per frame. |

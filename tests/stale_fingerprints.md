...
# R31-1: the C256 DOMINANCE family. Under the supervisor's option-1 framing C256 is a physical-layer
# comparator excluded from the deployed set BY DESIGN (modulation order is a transport parameter,
# not a semantic granularity). No dominance / zero-support / global-domination claim may be made,
# and the lambda argument in III-G was directionally wrong: C256 is the SMALLER payload, so a
# payload penalty biases toward it, not away.
RX C_?\{?256\}?\$? is (thus |therefore )?dominated
RX dominated by \$?C_?\{?16\}?
RX (256-QAM|C_\{256\}) is dominated by (16-QAM|C_\{16\})
RX zero support
RX globally dominated by the deployed set
RX \$C_\{16\}\$ dominates \$C_\{256\}\$

# R31-3: the pre-corrigendum R9 numbers that the LOCKED WORDING file was still carrying
# (results/provenance/r9_result_claims.md, now generated by tools/build_r9_claims.py). The file is
# the authority the ledger defers to, so a stale copy silently authorised these.
RX 56\.3\\?%
RX (dF|.F)[^\n]{0,20}-0\.0028(?![0-9])
RX -0\.0099(?![0-9])

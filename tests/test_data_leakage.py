...
# --- end bootstrap ---
from scene_split import scene_labels, _scene_frame_counts  # noqa: E402
...
def protocol_candidate_block():
    """Return (md5, n_candidates) for docs/experiment_protocol.md's CATOSG-CANDIDATES block (single source of truth)."""
    txt = open(PROTOCOL, encoding='utf-8').read()
    m = re.search(r'```json CATOSG-CANDIDATES\s*(\{.*?\})\s*```', txt, re.S)
    if not m:
        raise Fail('CATOSG-CANDIDATES block not found in docs/experiment_protocol.md')
...
def _grid(split):
    ...
    if not os.path.exists(p):
        raise Fail(f'{split} grid absent ({p}); run grid_builder.py (cannot verify without data).')
...
def check_scene_manifest_crosscheck():
    ...
    if not os.path.exists(man_p):
        raise Fail(f'scene manifest absent ({man_p}); run scene_split.py (no-skip anchor).')

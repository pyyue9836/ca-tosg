...
the three is wrong is a human call, and this tool deliberately does not make it.
    ...
    python tools/check_figure_consistency.py [--check]
"""
...
def main() -> int:
    ...
    report = buf.getvalue()
    # R44-4: this tool used to only ever REPORT, which is right for the one-sided and
    # different-condition rows -- those are readings, not defects. "Drawn but never stated" is not a
    # reading: it means a figure shows a number that no caption and no sentence in either delivered
    # document says, which is the same silent-drift class every other gate here exists for. --check
    # therefore fails on two things and nothing else: a stale committed report, and a non-empty
    # never-stated set.
    if '--check' in sys.argv:
        stale = (not os.path.exists(out)) or open(out, encoding='utf-8').read() != report
        why = []
        if missing_both:
            why.append('%d figure number(s) drawn but stated nowhere: %s'
                       % (len(missing_both), ', '.join(k for k, _v in missing_both)))
        if stale:
            why.append('docs/figure_text_consistency.md is stale -- re-run '
                       'python tools/check_figure_consistency.py')
        if why:
            print('FIGURE CONSISTENCY CHECK FAIL: ' + '; '.join(why))
            return 1
        print('FIGURE CONSISTENCY CHECK PASS: 0 drawn-but-never-stated numbers, report up to date.')
        return 0

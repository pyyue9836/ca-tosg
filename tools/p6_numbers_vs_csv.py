...
def self_test():
    ...
    # R68: the --check drift arm needs its own control, or "report is current" is an untested claim.
    text = build_report()[0]
    on_disk = read(OUT) if os.path.exists(OUT) else ''
    current = (on_disk == text)
    drift_caught = (on_disk + '\nDRIFT\n') != text
    print(f'  committed report matches a fresh build:       '
          f'{"OK" if current else "STALE (re-run the tool)"}')
    print(f'  a one-line drift is detected:                 '
          f'{"FIRES" if drift_caught else "DOES NOT FIRE"}')
    return 0 if (fired and quiet and current and drift_caught) else 1


def build_report():
    """Build the report text in memory and return (text, hits, misses, derived, skipped,
    t_found, t_missing, t_derived).

    R68: this used to be inlined in main(), which always wrote the file and always returned 0 --
    so a MISS was recorded in a report nobody re-generated and no gate read. The committed report
    had been stale since R57 and carried `MISS 0` while a fresh run said `MISS 1`. Splitting the
    build out is what lets `--check` compare disk against a fresh build without writing anything.
    """
    import io
    ...
    return (f.getvalue(), hits, misses, derived, skipped, t_found, t_missing, t_derived)
...
def main() -> int:
    ...
    text, hits, misses, derived, skipped, t_found, t_missing, t_derived = build_report()
    ...
    rc = 0
    # R68 (1): a MISS or an unlocated table cell is a FAILURE, not a line in a report. The tool
    # returned 0 unconditionally, so `MISS 1` could and did sit unread for ten batches.
    if misses:
        print(f'P6 GATE FAIL: {len(misses)} bound literal(s) are in no file the ledger names')
        rc = 1
    if t_missing:
        print(f'P6 GATE FAIL: {len(t_missing)} table cell(s) are in no committed product')
        rc = 1
    # R68 (2): and the committed report must match a fresh build -- same family as
    # tools/check_figure_consistency.py --check. A stale report is a stale claim about the tree.
    on_disk = read(OUT) if os.path.exists(OUT) else None
    if check_only:
        if on_disk is None:
            print(f'P6 GATE FAIL: {os.path.relpath(OUT, ROOT)} is missing -- run the tool')
            rc = 1
        elif on_disk != text:
            print(f'P6 GATE FAIL: {os.path.relpath(OUT, ROOT)} is STALE -- it does not match a '
                  'fresh build. Re-run `python tools/p6_numbers_vs_csv.py`.')
            rc = 1
        if rc == 0:
            print('P6 GATE PASS: report current, 0 MISS, 0 unlocated table cells')
    else:
